...
roc.rfe.1 <- roc(Prediction_rfe.1$Class,                    # response vector (factor or character)
...
roc.GA.2 <- roc(Prediction_GA.2$Class,                    # response vector (factor or character)
...
Prediction_GA.2$Class_Code <- ifelse(Prediction_GA.2$TrueLabel == "transforming",1,0)
pr.GA.2 <- pr.curve(scores.class0 = Prediction_GA.2$transforming , weights.class0 = Prediction_GA.2$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.GA.2, rand.plot = TRUE, legend = F, color = 1,main = "")
...
roc.rfe.5 <- roc(Prediction_rfe.5$Class,                    # response vector (factor or character)
...
optFeatures.rfe.9 <- cbind(rfe.9$optVariables, Annotation[rfe.9$optVariables,])
write.table(optFeatures.rfe.9, file = "TrainingSet09_optVars_rfe.txt", sep="\t",col.names=NA)
...
### Figure 4b plot external accuracy over the iterations ################################################################
...
### Figure 4c: PCA on 11 probes from SVM-GA
...
### Figure 4d: PCA on 11 random probes 
index.4e      <- sample(36226,11, replace = FALSE)
matrix.random <- eset.batch[index.4e,]
...
Results <- data.frame(Testset = c(seq(1,10,by=1),"FinalSet152"), FeatureNumber_full = rep(NA,11), ResamplingAccuracy_full=rep(NA,11),CI_full_lower=rep(NA,11),CI_full_upper=rep(NA,11), 
                      ...
                      Accuracy_TestSet_SAGA = rep(NA,11),AUROC_SAGA = rep(NA,11), AUPRC_SAGA = rep(NA,11),AUPRC_random = rep(NA,11))
...
### 2.24.TestSet accuracy for SAGA = TestSet accuracy of rfe (if <10 features found by rfe) and GA otherwise 
for (i in c(2,3,6,7,8,10)) { Results[i,24] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[1] }
for (i in c(1,4,5,9)) { Results[i,24]      <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_rfe.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_rfe.",i, sep="")))$TrueLabel))$overall[1] }

### 2.25. TestSet AUROC for SAGA
for (i in c(2,3,6,7,8,10)) { Results[i,25] <- round(eval(parse(text=paste("roc.GA.",i, sep="")))$auc[1]/100,3) }
for (i in c(1,4,5,9))      { Results[i,25] <- round(eval(parse(text=paste("roc.rfe.",i, sep="")))$auc[1]/100,3) }
Results[2,19] <- round(roc.rfe.2$auc[1]/100,3)

### 2.26. TestSet AUPRC for SAGA
for (i in c(2,3,6,7,8,10)) { Results[i,26] <- round(eval(parse(text=paste("pr.GA.",i, sep="")))$auc.integral,3)}
for (i in c(1,4,5,9))      { Results[i,26] <- round(eval(parse(text=paste("pr.rfe.",i, sep="")))$auc.integral,3)}

### 2.27. TestSet AUPRC for a random classifier
for (i in c(2,3,6,7,8,10)) { Results[i,27] <- round(eval(parse(text=paste("pr.GA.",i, sep="")))$rand$auc.integral,3)}
for (i in c(1,4,5,9))      { Results[i,27] <- round(eval(parse(text=paste("pr.rfe.",i, sep="")))$rand$auc.integral,3)}
...
#############################################################################################################################################
### 4. aggregate Test set performances over the 10 different splits  ########################################################################
#############################################################################################################################################
...
sink("ConfusionMatrix_IVIM_all 502 samples_.txt", append = TRUE)
...
# AUCROC for RFE and GA separately (not shown)
...
### 6.4.Supplementary Figure 5b IVIM vs SAGA on LTR.RV.SF only ##############################################################################
#############################################################################################################################################
...
### 6.5.Supplementary Figure 5c IVIM vs SAGA non-LTR vectors ################################################################################
#############################################################################################################################################
...
#############################################################################################################################################
### 7 Precision-Recall curve for IVIM vs SAGA: F ############################################################################################
#############################################################################################################################################

### 7.1 for all vectors: Figure 3h  #########################################################################################################
...
plot(AUPRC.SAGA, rand.plot = TRUE, legend = F, color = "#F9C35F", main = "",auc.main = FALSE)  # AUPRC =  0.937  / AUC random = 0.422
...
### 7.2 for LTR.SF Supplementary Figure 5b lower panel #######################################################################################
...
### 2.6.2.3 for non-LTR.SF Supplementary Figure 5c lower panel ###############################################################################
...
plot(AUPRC.SAGA.other, rand.plot = TRUE, legend = F, color = "#F9C35F", main = "")  # AUPRC = 0.763276  / AUC random = 0.215625 
...
#############################################################################################################################################
### 8. AUCROC & PRROC SAGA vs IVIM (only the 152 samples for which SAGA and IVIM are available ): Supplementary Figure 5d,e,f################
#############################################################################################################################################
pData.152           <- pData
pData.152$TrueLabel <- pData$Class  
pData.152$Class     <- ifelse(pData.152$TrueLabel=="transforming","transforming","nontransforming")
pData.152$Class_Code<- ifelse(pData.152$TrueLabel=="transforming",1,0)
...
### 8.1.Supplementary Figure 5d: IVIM vs SAGA performance (for all vectors) #################################################################
#############################################################################################################################################
...
roc.test(roc.SAGA, IVIM.152, alternative = "greater") #p-value = 0.00514
...
AUPRC.IVIM.152 <- pr.curve(scores.class0 = pData.152$MTT_4, weights.class0 = as.numeric(pData.152$Class_Code), curve = TRUE,rand.compute = T)
plot(AUPRC.IVIM.152, rand.plot = TRUE, legend = F, color = "#8285BC", add = TRUE) # AUPRC = 0.892  / AUC random = 0.582


### 8.2.Supplementary Figure 5e IVIM vs SAGA on LTR.RV.SF only ##############################################################################
#############################################################################################################################################
...
roc.test(ROC.SAGA.LTR.SF, IVIM.LTR.152,alternative = "greater") #p-value = 0.09294


AUPRC.SAGA.LTR <- pr.curve(scores.class0 = Predictions_SAGA_LTR.SF$transforming , weights.class0 = Predictions_SAGA_LTR.SF$Class_Code, curve = TRUE,rand.compute = T)
plot(AUPRC.SAGA.LTR, rand.plot = TRUE, legend = F, color = "#F9C35F", main = "")  # AUPRC = 0.9993  / AUC random = 0.521

AUPRC.IVIM.LTR.152 <- pr.curve(scores.class0 = pData.LTR$MTT_4, weights.class0 = as.numeric(pData.LTR$Class_Code), curve = TRUE,rand.compute = T)
plot(AUPRC.IVIM.LTR.152, rand.plot = TRUE, legend = F, color = "#8285BC", add = TRUE) 



### 8.3.Supplementary Figure 5f IVIM vs SAGA other vectors ##################################################################################
#############################################################################################################################################
...
roc.test(ROC.SAGA.other, IVIM.noLTR.152,alternative = "greater") #p-value = 0.01993


AUPRC.SAGA.other <- pr.curve(scores.class0 = Predictions_SAGA_other$transforming , weights.class0 = Predictions_SAGA_other$Class_Code, curve = TRUE,rand.compute = T)
plot(AUPRC.SAGA.other, rand.plot = TRUE, legend = F, color = "#F9C35F", main = "")  

AUPRC.IVIM.noLTR.152 <- pr.curve(scores.class0 = pData.noLTR$MTT_4, weights.class0 = as.numeric(pData.noLTR$Class_Code), curve = TRUE,rand.compute = T)
plot(AUPRC.IVIM.noLTR.152, rand.plot = TRUE, legend = F, color = "#8285BC", add = TRUE) # PRROC =  0.6500137 # PRROC Random 0.2434783
...
#############################################################################################################################################
### 2.7. Toplist of predictors chosen during the 10 iterations Table 1 ######################################################################
#############################################################################################################################################

all.optVars <- c(optFeatures.rfe.1$GeneSymbol_FINAL,optVars.GA.2$GeneSymbol_FINAL,optVars.GA.3$GeneSymbol_FINAL,optVars.GA.6$GeneSymbol_FINAL,
                     optVars.GA.7$GeneSymbol_FINAL,optVars.GA.8$GeneSymbol_FINAL,optVars.GA.10$GeneSymbol_FINAL,optFeatures.rfe.4$GeneSymbol_FINAL,
                     optFeatures.rfe.5$GeneSymbol_FINAL,optFeatures.rfe.9$GeneSymbol_FINAL)

b <- table(all.optVars)
b <- b[order(b,decreasing = T)]

c <- subset(varImp_FINAL, !duplicated(varImp_FINAL$GeneSymbol_FINAL))
c$ProbeID <- row.names(c)
row.names(c) <- c$GeneSymbol_FINAL

d <- c[names(b),]
d <- cbind(d,b)
write.table(d, file="Toplist_optimal predictors_10testsets.txt", sep="\t")
