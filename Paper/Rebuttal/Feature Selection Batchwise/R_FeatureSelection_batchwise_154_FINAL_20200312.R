...
#### 7.1 Addon-quantile normalization ######################################################################## 
##############################################################################################################
...
#### 7.2 Addon COMBAT batch correction ####################################################################### 
##############################################################################################################
...
#### 7.3 subset matrices for the optimal variables ###########################################################
##############################################################################################################
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.1.bapred.GA, matrix.test.1.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.1$Design_Color,pData.test.1$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.1)+1):154),c(1:2)], labels=row.names(pData.test.1), cex= 0.4, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet01 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet01 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet01 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.2    <- qunormtrain(t(RAW_train.2$E))                                # create qunormtrain object 
matrix.train.2.qn <- log2(t(qunorm.train.2$xnorm))                                # extract quantile-normalized training matrix
matrix.train.2.qn <- avereps(matrix.train.2.qn, ID= RAW_train.2$genes$ProbeName)  # average quadruplicates  
...
combat.train.2        <- combatba(t(matrix.train.2.qn), batch = batch.train.2)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.2.bapred.rfe, matrix.test.2.bapred.rfe))           
plot(pca$x, pch=16, col=c(pData.train.2$Design_Color,pData.test.2$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.2)+1):154),c(1:2)], labels=row.names(pData.test.2), cex= 0.5, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet02 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet02 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 02 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
...
#### VIII.2 ROC on probability "transforming" TestSet 02 ###################################################
...
#### VIII.3 Precision-Recall Curve on probability "transforming" TestSet 02 ################################
...
#### 7.3 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.3    <- qunormtrain(t(RAW_train.3$E))                                # create qunormtrain object 
matrix.train.3.qn <- log2(t(qunorm.train.3$xnorm))                                # extract quantile-normalized training matrix
matrix.train.3.qn <- avereps(matrix.train.3.qn, ID= RAW_train.3$genes$ProbeName)  # average quadruplicates  
...
#### 7.3 Addon COMBAT batch correction ####################################################################### 
##############################################################################################################
batch.train.3         <- as.factor(ifelse(pData.train.3$Batch>3,pData.train.3$Batch-1,pData.train.3$Batch))
combat.train.3        <- combatba(t(matrix.train.3.qn), batch = batch.train.3)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.3.bapred.GA, matrix.test.3.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.3$Design_Color,pData.test.3$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.3)+1):154),c(1:2)], labels=row.names(pData.test.3), cex= 0.3, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet03 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet03 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet03 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 03 ##################################################
#############################################################################################################
...
#### VIII.2 ROC on probability "transforming" TestSet 03 ########################################################
...
### Precision-Recall curve on TestSet 03 ##########################################################
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.4    <- qunormtrain(t(RAW_train.4$E))                                # create qunormtrain object 
matrix.train.4.qn <- log2(t(qunorm.train.4$xnorm))                                # extract quantile-normalized training matrix
matrix.train.4.qn <- avereps(matrix.train.4.qn, ID= RAW_train.4$genes$ProbeName)  # average quadruplicates  
...
combat.train.4        <- combatba(t(matrix.train.4.qn), batch = batch.train.4)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.4.bapred.GA, matrix.test.4.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.4$Design_Color,pData.test.4$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.4)+1):154),c(1:2)], labels=row.names(pData.test.4), cex= 0.3, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet04 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet04 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet04 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 04 ##################################################
#############################################################################################################
...
#### VIII.2 ROC on probability "transforming" TestSet 04 ########################################################
...
### Precision-Recall curve on TestSet 04 ##########################################################
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.5    <- qunormtrain(t(RAW_train.5$E))                                # create qunormtrain object 
matrix.train.5.qn <- log2(t(qunorm.train.5$xnorm))                                # extract quantile-normalized training matrix
matrix.train.5.qn <- avereps(matrix.train.5.qn, ID= RAW_train.5$genes$ProbeName)  # average quadruplicates  
...
combat.train.5        <- combatba(t(matrix.train.5.qn), batch = batch.train.5)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.5.bapred.GA, matrix.test.5.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.5$Design_Color,pData.test.5$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.5)+1):154),c(1:2)], labels=row.names(pData.test.5), cex= 0.3, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet05 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet05 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet05 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 05 ##################################################
#############################################################################################################
...
#### VIII.2 ROC on probability "transforming" TestSet 05 ########################################################
Prediction_rfe.5$Class <- as.factor(ifelse(Prediction_rfe.5$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet05_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet05_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet05_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 05 ##########################################################
...
pdf("TestSet05_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet05_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet05_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.6    <- qunormtrain(t(RAW_train.6$E))                                # create qunormtrain object 
matrix.train.6.qn <- log2(t(qunorm.train.6$xnorm))                                # extract quantile-normalized training matrix
matrix.train.6.qn <- avereps(matrix.train.6.qn, ID= RAW_train.6$genes$ProbeName)  # average quadruplicates  
...
combat.train.6        <- combatba(t(matrix.train.6.qn), batch = batch.train.6)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.6.bapred.GA, matrix.test.6.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.6$Design_Color,pData.test.6$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.6)+1):154),c(1:2)], labels=row.names(pData.test.6), cex= 0.4, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet06 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet06 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet06 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 06 ##################################################
#############################################################################################################
...
#### VIII.2 ROC on probability "transforming" TestSet 06 ########################################################
Prediction_rfe.6$Class <- as.factor(ifelse(Prediction_rfe.6$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet06_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet06_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet06_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 06 ##########################################################
...
pdf("TestSet06_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet06_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet06_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.7    <- qunormtrain(t(RAW_train.7$E))                                # create qunormtrain object 
matrix.train.7.qn <- log2(t(qunorm.train.7$xnorm))                                # extract quantile-normalized training matrix
matrix.train.7.qn <- avereps(matrix.train.7.qn, ID= RAW_train.7$genes$ProbeName)  # average quadruplicates  
...
combat.train.7        <- combatba(t(matrix.train.7.qn), batch = batch.train.7)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.7.bapred.GA, matrix.test.7.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.7$Design_Color,pData.test.7$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.7)+1):154),c(1:2)], labels=row.names(pData.test.7), cex= 0.4, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet07 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet07 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet07 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 07 ##################################################
#############################################################################################################
...
#### VIII.2 ROC on probability "transforming" TestSet 07 ########################################################

Prediction_rfe.7$Class <- as.factor(ifelse(Prediction_rfe.7$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet07_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet07_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet07_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 07 ##########################################################
...
pdf("TestSet07_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet07_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet07_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.8    <- qunormtrain(t(RAW_train.8$E))                                # create qunormtrain object 
matrix.train.8.qn <- log2(t(qunorm.train.8$xnorm))                                # extract quantile-normalized training matrix
matrix.train.8.qn <- avereps(matrix.train.8.qn, ID= RAW_train.8$genes$ProbeName)  # average quadruplicates  
...
combat.train.8        <- combatba(t(matrix.train.8.qn), batch = batch.train.8)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.8.bapred.GA, matrix.test.8.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.8$Design_Color,pData.test.8$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.8)+1):154),c(1:2)], labels=row.names(pData.test.8), cex= 0.4, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet08 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet08 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet08 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 08 ##################################################
#############################################################################################################
...
#### VIII.2 ROC on probability "transforming" TestSet 08 ########################################################
Prediction_rfe.8$Class <- as.factor(ifelse(Prediction_rfe.8$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet08_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet08_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet08_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 08 ##########################################################
...
pdf("TestSet08_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet08_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet08_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.9    <- qunormtrain(t(RAW_train.9$E))                                # create qunormtrain object 
matrix.train.9.qn <- log2(t(qunorm.train.9$xnorm))                                # extract quantile-normalized training matrix
matrix.train.9.qn <- avereps(matrix.train.9.qn, ID= RAW_train.9$genes$ProbeName)  # average quadruplicates  
...
combat.train.9        <- combatba(t(matrix.train.9.qn), batch = batch.train.9)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.9.bapred.GA, matrix.test.9.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.9$Design_Color,pData.test.9$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.9)+1):154),c(1:2)], labels=row.names(pData.test.9), cex= 0.4, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet09 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet09 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet09 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 09 ##################################################
#############################################################################################################
...
#### VIII.2 ROC on probability "transforming" TestSet 09 ########################################################
Prediction_rfe.9$Class <- as.factor(ifelse(Prediction_rfe.9$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet09_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet09_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet09_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 09 ##########################################################
...
pdf("TestSet09_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet09_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet09_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.10    <- qunormtrain(t(RAW_train.10$E))                                # create qunormtrain object 
matrix.train.10.qn <- log2(t(qunorm.train.10$xnorm))                                # extract quantile-normalized training matrix
matrix.train.10.qn <- avereps(matrix.train.10.qn, ID= RAW_train.10$genes$ProbeName)  # average quadruplicates  
...
combat.train.10        <- combatba(t(matrix.train.10.qn), batch = batch.train.10)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.10.bapred.GA, matrix.test.10.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.10$Design_Color,pData.test.10$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.10)+1):154),c(1:2)], labels=row.names(pData.test.10), cex= 0.4, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet010 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
#### 7.6 train SVM and predict TestSet100 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
##### 7.7 Predict TestSet100 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
#### VIII Performance of optVars Classifier on  TestSet 010 ##################################################
#############################################################################################################
...
#### VIII.2 ROC on probability "transforming" TestSet 010 ########################################################
Prediction_rfe.10$Class <- as.factor(ifelse(Prediction_rfe.10$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet010_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet010_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet010_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 010 ##########################################################
...
pdf("TestSet010_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet010_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet010_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.11    <- qunormtrain(t(RAW_train.11$E))                                # create qunormtrain object 
matrix.train.11.qn <- log2(t(qunorm.train.11$xnorm))                                # extract quantile-normalized training matrix
matrix.train.11.qn <- avereps(matrix.train.11.qn, ID= RAW_train.11$genes$ProbeName)  # average quadruplicates  
...
combat.train.11        <- combatba(t(matrix.train.11.qn), batch = batch.train.11)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.11.bapred.GA, matrix.test.11.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.11$Design_Color,pData.test.11$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.11)+1):154),c(1:2)], labels=row.names(pData.test.11), cex= 0.4, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet011 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.11, file = paste("TestSet11_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet011 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_GA.11, file = paste("TestSet11_Predictions_optVars_GA.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)


##### 7.7 Predict TestSet011 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.11, file = paste("TestSet11_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 011 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet11_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.11$Prediction_rfe.11), as.factor(Prediction_rfe.11$TrueLabel))
sink()

sink("TestSet11_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.11$Prediction_GA.11), as.factor(Prediction_GA.11$TrueLabel))
sink()


sink("TestSet11_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.11$Prediction_SVM_full), as.factor(Prediction_SVM_full.11$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 011 ########################################################

Prediction_rfe.11$Class <- as.factor(ifelse(Prediction_rfe.11$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet11_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet11_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet11_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 11 ##########################################################
...
pdf("TestSet11_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet11_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet11_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.12    <- qunormtrain(t(RAW_train.12$E))                                # create qunormtrain object 
matrix.train.12.qn <- log2(t(qunorm.train.12$xnorm))                                # extract quantile-normalized training matrix
matrix.train.12.qn <- avereps(matrix.train.12.qn, ID= RAW_train.12$genes$ProbeName)  # average quadruplicates  
...
combat.train.12        <- combatba(t(matrix.train.12.qn), batch = batch.train.12)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.12.bapred.GA, matrix.test.12.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.12$Design_Color,pData.test.12$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.12)+1):154),c(1:2)], labels=row.names(pData.test.12), cex= 0.3, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet012 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.12, file = paste("TestSet12_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet122 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_GA.12, file = paste("TestSet12_Predictions_optVars_GA.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)


##### 7.7 Predict TestSet122 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.12, file = paste("TestSet12_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 012 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet12_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.12$Prediction_rfe.12), as.factor(Prediction_rfe.12$TrueLabel))
sink()

sink("TestSet12_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.12$Prediction_GA.12), as.factor(Prediction_GA.12$TrueLabel))
sink()


sink("TestSet12_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.12$Prediction_SVM_full), as.factor(Prediction_SVM_full.12$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 012 ########################################################
Prediction_rfe.12$Class <- as.factor(ifelse(Prediction_rfe.12$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet12_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet12_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet12_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 12 ##########################################################
...
pdf("TestSet12_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet12_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet12_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.13    <- qunormtrain(t(RAW_train.13$E))                                # create qunormtrain object 
matrix.train.13.qn <- log2(t(qunorm.train.13$xnorm))                                # extract quantile-normalized training matrix
matrix.train.13.qn <- avereps(matrix.train.13.qn, ID= RAW_train.13$genes$ProbeName)  # average quadruplicates  
...
combat.train.13        <- combatba(t(matrix.train.13.qn), batch = batch.train.13)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.13.bapred.GA, matrix.test.13.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.13$Design_Color,pData.test.13$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.13)+1):154),c(1:2)], labels=row.names(pData.test.13), cex= 0.3, pos=3, offset = 0.3) 


#### 7.5 train SVM and predict TestSet013 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.13, file = paste("TestSet13_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet13 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_GA.13, file = paste("TestSet13_Predictions_optVars_GA.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)


##### 7.7 Predict TestSet133 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.13, file = paste("TestSet13_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 013 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet13_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.13$Prediction_rfe.13), as.factor(Prediction_rfe.13$TrueLabel))
sink()

sink("TestSet13_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.13$Prediction_GA.13), as.factor(Prediction_GA.13$TrueLabel))
sink()


sink("TestSet13_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.13$Prediction_SVM_full), as.factor(Prediction_SVM_full.13$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 013 ########################################################

Prediction_rfe.13$Class <- as.factor(ifelse(Prediction_rfe.13$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet13_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet13_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet13_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 13 ##########################################################
...
pdf("TestSet13_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet13_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet13_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.14    <- qunormtrain(t(RAW_train.14$E))                                # create qunormtrain object 
matrix.train.14.qn <- log2(t(qunorm.train.14$xnorm))                                # extract quantile-normalized training matrix
matrix.train.14.qn <- avereps(matrix.train.14.qn, ID= RAW_train.14$genes$ProbeName)  # average quadruplicates  
...
combat.train.14        <- combatba(t(matrix.train.14.qn), batch = batch.train.14)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.14.bapred.GA, matrix.test.14.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.14$Design_Color,pData.test.14$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.14)+1):154),c(1:2)], labels=row.names(pData.test.14), cex= 0.3, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet014 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.14, file = paste("TestSet14_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet144 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_GA.14, file = paste("TestSet14_Predictions_optVars_GA.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)


##### 7.7 Predict TestSet144 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.14, file = paste("TestSet14_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 014 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet14_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.14$Prediction_rfe.14), as.factor(Prediction_rfe.14$TrueLabel))
sink()

sink("TestSet14_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.14$Prediction_GA.14), as.factor(Prediction_GA.14$TrueLabel))
sink()


sink("TestSet14_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.14$Prediction_SVM_full), as.factor(Prediction_SVM_full.14$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 014 ########################################################

Prediction_rfe.14$Class <- as.factor(ifelse(Prediction_rfe.14$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet14_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet14_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet14_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 14 ##########################################################
...
pdf("TestSet14_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet14_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet14_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.15    <- qunormtrain(t(RAW_train.15$E))                                # create qunormtrain object 
matrix.train.15.qn <- log2(t(qunorm.train.15$xnorm))                                # extract quantile-normalized training matrix
matrix.train.15.qn <- avereps(matrix.train.15.qn, ID= RAW_train.15$genes$ProbeName)  # average quadruplicates  
...
combat.train.15        <- combatba(t(matrix.train.15.qn), batch = batch.train.15)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.15.bapred.GA, matrix.test.15.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.15$Design_Color,pData.test.15$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.15)+1):154),c(1:2)], labels=row.names(pData.test.15), cex= 0.4, pos=3, offset = 0.3) 


#### 7.5 train SVM and predict TestSet015 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.15, file = paste("TestSet15_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet155 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_GA.15, file = paste("TestSet15_Predictions_optVars_GA.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)


##### 7.7 Predict TestSet15 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.15, file = paste("TestSet15_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 015 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet15_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.15$Prediction_rfe.15), as.factor(Prediction_rfe.15$TrueLabel))
sink()

sink("TestSet15_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.15$Prediction_GA.15), as.factor(Prediction_GA.15$TrueLabel))
sink()


sink("TestSet15_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.15$Prediction_SVM_full), as.factor(Prediction_SVM_full.15$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 015 ########################################################
Prediction_rfe.15$Class <- as.factor(ifelse(Prediction_rfe.15$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet15_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet15_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet15_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 15 ##########################################################
...
pdf("TestSet15_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet15_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet15_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.16    <- qunormtrain(t(RAW_train.16$E))                                # create qunormtrain object 
matrix.train.16.qn <- log2(t(qunorm.train.16$xnorm))                                # extract quantile-normalized training matrix
matrix.train.16.qn <- avereps(matrix.train.16.qn, ID= RAW_train.16$genes$ProbeName)  # average quadruplicates  
...
combat.train.16        <- combatba(t(matrix.train.16.qn), batch = batch.train.16)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.16.bapred.GA, matrix.test.16.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.16$Design_Color,pData.test.16$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.16)+1):154),c(1:2)], labels=row.names(pData.test.16), cex= 0.3, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet016 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.16, file = paste("TestSet16_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet166 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_GA.16, file = paste("TestSet16_Predictions_optVars_GA.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)


##### 7.7 Predict TestSet166 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.16, file = paste("TestSet16_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 016 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet16_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.16$Prediction_rfe.16), as.factor(Prediction_rfe.16$TrueLabel))
sink()

sink("TestSet16_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.16$Prediction_GA.16), as.factor(Prediction_GA.16$TrueLabel))
sink()


sink("TestSet16_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.16$Prediction_SVM_full), as.factor(Prediction_SVM_full.16$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 016 ########################################################
Prediction_rfe.16$Class <- as.factor(ifelse(Prediction_rfe.16$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet16_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet16_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet16_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 16 ##########################################################
...
pdf("TestSet16_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet16_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet16_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.17    <- qunormtrain(t(RAW_train.17$E))                                # create qunormtrain object 
matrix.train.17.qn <- log2(t(qunorm.train.17$xnorm))                                # extract quantile-normalized training matrix
matrix.train.17.qn <- avereps(matrix.train.17.qn, ID= RAW_train.17$genes$ProbeName)  # average quadruplicates  
...
combat.train.17        <- combatba(t(matrix.train.17.qn), batch = batch.train.17)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.17.bapred.GA, matrix.test.17.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.17$Design_Color,pData.test.17$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.17)+1):154),c(1:2)], labels=row.names(pData.test.17), cex= 0.3, pos=3, offset = 0.3) 


#### 7.5 train SVM and predict TestSet017 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.17, file = paste("TestSet17_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet177 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_GA.17, file = paste("TestSet17_Predictions_optVars_GA.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)


##### 7.7 Predict TestSet17 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.17, file = paste("TestSet17_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 017 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet17_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.17$Prediction_rfe.17), as.factor(Prediction_rfe.17$TrueLabel))
sink()

sink("TestSet17_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.17$Prediction_GA.17), as.factor(Prediction_GA.17$TrueLabel))
sink()


sink("TestSet17_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.17$Prediction_SVM_full), as.factor(Prediction_SVM_full.17$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 017 ########################################################
Prediction_rfe.17$Class <- as.factor(ifelse(Prediction_rfe.17$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet17_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet17_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet17_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 17 ##########################################################
...
pdf("TestSet17_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet17_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet17_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.18    <- qunormtrain(t(RAW_train.18$E))                                # create qunormtrain object 
matrix.train.18.qn <- log2(t(qunorm.train.18$xnorm))                                # extract quantile-normalized training matrix
matrix.train.18.qn <- avereps(matrix.train.18.qn, ID= RAW_train.18$genes$ProbeName)  # average quadruplicates  
...
combat.train.18        <- combatba(t(matrix.train.18.qn), batch = batch.train.18)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.18.bapred.GA, matrix.test.18.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.18$Design_Color,pData.test.18$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.18)+1):154),c(1:2)], labels=row.names(pData.test.18), cex= 0.3, pos=3, offset = 0.3) 


#### 7.5 train SVM and predict TestSet018 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.18, file = paste("TestSet18_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet188 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_GA.18, file = paste("TestSet18_Predictions_optVars_GA.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)


##### 7.7 Predict TestSet188 with all predictors ##############################################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.18, file = paste("TestSet18_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 018 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet18_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.18$Prediction_rfe.18), as.factor(Prediction_rfe.18$TrueLabel))
sink()

sink("TestSet18_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.18$Prediction_GA.18), as.factor(Prediction_GA.18$TrueLabel))
sink()


sink("TestSet18_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.18$Prediction_SVM_full), as.factor(Prediction_SVM_full.18$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 018 ########################################################
Prediction_rfe.18$Class <- as.factor(ifelse(Prediction_rfe.18$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet18_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet18_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet18_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 18 ##########################################################
...
pdf("TestSet18_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet18_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet18_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
#### 7.1 Addon quantile normalization ######################################################################## 
##############################################################################################################
qunorm.train.19    <- qunormtrain(t(RAW_train.19$E))                                # create qunormtrain object 
matrix.train.19.qn <- log2(t(qunorm.train.19$xnorm))                                # extract quantile-normalized training matrix
matrix.train.19.qn <- avereps(matrix.train.19.qn, ID= RAW_train.19$genes$ProbeName)  # average quadruplicates  
...
combat.train.19        <- combatba(t(matrix.train.19.qn), batch = batch.train.19)   # create combatba object 
...
#### 7.4 PCA on best predictors found by genetic algorithm / rfe #############################################
##############################################################################################################
pca <- prcomp(rbind(matrix.train.19.bapred.GA, matrix.test.19.bapred.GA))           
plot(pca$x, pch=16, col=c(pData.train.19$Design_Color,pData.test.19$Design_Color), cex=1, asp=1)
text(pca$x[c((nrow(pData.train.19)+1):154),c(1:2)], labels=row.names(pData.test.19), cex= 0.3, pos=3, offset = 0.3) 

#### 7.5 train SVM and predict TestSet019 on optVars found by rfe  ############################################
##############################################################################################################
set.seed(721)
...
write.table(Prediction_rfe.19, file = paste("TestSet19_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.6 train SVM and predict TestSet199 on optVars found by GA  ############################################
#############################################################################################################
set.seed(721)
...
write.table(Prediction_SVM_full.19, file = paste("TestSet19_Predictions_allVars.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### VIII Performance of optVars Classifier on  TestSet 019 ##################################################
#############################################################################################################

#### VIII.1 Confusion matrix ################################################################################  
sink("TestSet19_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.19$Prediction_rfe.19), as.factor(Prediction_rfe.19$TrueLabel))
sink()

sink("TestSet19_ConfusionMatrix_GA.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_GA.19$Prediction_GA.19), as.factor(Prediction_GA.19$TrueLabel))
sink()


sink("TestSet19_ConfusionMatrix_FULL.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_SVM_full.19$Prediction_SVM_full), as.factor(Prediction_SVM_full.19$TrueLabel))
sink()

#### VIII.2 ROC on probability "transforming" TestSet 019 ########################################################
Prediction_rfe.19$Class <- as.factor(ifelse(Prediction_rfe.19$TrueLabel == "transforming","transforming","nontransforming"))
pdf("TestSet19_ROC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet19_ROC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet19_ROC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
### Precision-Recall curve on TestSet 19 ##########################################################
...
pdf("TestSet19_PRC_optVars_rfe.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet19_PRC_optVars_GA.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
pdf("TestSet19_PRC_optVars_full.pdf", width=5.71, height=5.68, useDingbats = F, fonts = "sans")
...
Results <- data.frame(Testset = c(seq(1,19,by=1)), FeatureNumber_full = rep(NA,19), ResamplingAccuracy_full=rep(NA,19),CI_full_lower=rep(NA,19),CI_full_upper=rep(NA,19), 
...
for (i in 1:19) {Results[i,4] <- round(ci(eval(parse(text=paste("rfeResamples.",i, sep="")))$values[,2])[2],3)}
...
for (i in 1:19) {Results[i,5] <- round(ci(eval(parse(text=paste("rfeResamples.",i, sep="")))$values[,2])[3],3)}

### 6. number of features found by SVM-rfe for each training/test split
for (i in 1:19) {Results[i,6] <- eval(parse(text=paste("rfe.",i, sep="")))$optsize }

### 7. external resampling accuracy for the svm-rfe model
for (i in 1:19) { Results[i,7] <- round(eval(parse(text=paste("rfe.",i, sep="")))$results[eval(parse(text=paste("rfe.",i, sep="")))$results$Variables == eval(parse(text=paste("rfe.",i, sep="")))$optsize,5],3)}
...
for (i in 1:19) {Results[i,8] <- round(ci(eval(parse(text=paste("rfeResamples.",i, sep="")))$values[,7])[2],3)}

### 9. upper bound confidence intervall for the resampling accuracy of rfe model / stored in rfeResamples.x$values dataframe
for (i in 1:19) {Results[i,9] <- round(ci(eval(parse(text=paste("rfeResamples.",i, sep="")))$values[,7])[3],3)}
...
for (i in 1:16) {Results[i,10] <- eval(parse(text=paste("modelDifferences.",i, sep="")))$statistics$Accuracy[[1]][3]}
Results[17,10] <- NA
for (i in 18:19) {Results[i,10] <- eval(parse(text=paste("modelDifferences.",i, sep="")))$statistics$Accuracy[[1]][3]}

### 11. number of features found by SVM-GA 
for (i in c(1,3:19)) { Results[i,11] <- length(eval(parse(text=paste("GA.",i, sep="")))$optVariables)  }

### 12. Resampling accuracy for the GA model
for (i in c(1,3:19)) { Results[i,12] <- round(eval(parse(text=paste("accuracy.external.",i, sep=""))),3)}
...
for (i in c(1,3:19)) {Results[i,13] <- round(ci(eval(parse(text=paste("accuracy.external.opt.",i, sep=""))))[2],3)}
...
for (i in c(1,3:19)) {Results[i,14] <- round(ci(eval(parse(text=paste("accuracy.external.opt.",i, sep=""))))[3],3)}

### 15. TestSet accuracy for the full model
for (i in 1:19) {Results[i,15] <- round(confusionMatrix(as.factor(eval(parse(text=paste("Prediction_SVM_full.",i, sep="")))$Prediction_SVM_full), as.factor(eval(parse(text=paste("Prediction_SVM_full.",i, sep="")))$TrueLabel))$overall[1],3)}

### 16. TestSet AUROC for the full model
for (i in 1:19) {Results[i,16] <- round(eval(parse(text=paste("roc.full.",i, sep="")))$auc[1]/100,3) }

### 17. TestSet AUPRC for the full model
for (i in 1:19) {Results[i,17] <- round(eval(parse(text=paste("pr.full.",i, sep="")))$auc.integral,3)}

### 18. TestSet accuracy for SAGA = TestSet accuracy of rfe (TestSet 02; <10 features found by rfe) and GA otherwise (all other TestSets)
for (i in c(1,3:19)) { Results[i,18] <- round(confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[1],3)}
Results[2,18] <- round(confusionMatrix(as.factor(eval(parse(text=paste("Prediction_rfe.",2, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_rfe.",2, sep="")))$TrueLabel))$overall[1],3)

### 19. TestSet AUROC for SAGA
for (i in c(1,3:19)) { Results[i,19] <- round(eval(parse(text=paste("roc.GA.",i, sep="")))$auc[1]/100,3) }
Results[2,19] <- round(roc.rfe.2$auc[1]/100,3)

### 20. TestSet AUPRC for SAGA
for (i in c(1,3:19)) { Results[i,20] <- round(eval(parse(text=paste("pr.GA.",i, sep="")))$auc.integral,3)}
Results[2,20] <- round(pr.rfe.2$auc.integral,3)

### 21. TestSet AUPRC for a random classifier
for (i in 1:19) { Results[i,21] <- round(eval(parse(text=paste("pr.full.",i, sep="")))$rand$auc.integral,3)}
