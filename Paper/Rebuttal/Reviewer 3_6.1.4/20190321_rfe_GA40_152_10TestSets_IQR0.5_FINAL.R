...
for (i in c(1,3,6)) { Results[i,6] <- length(eval(parse(text=paste("GA.",i, sep="")))$optVariables)  }
...
for (i in c(1,3,6)) { Results[i,9] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[1] }
