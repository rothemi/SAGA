...
# note: these resampling values seem to be better than the external resampling results (above) 
# due to selection bias since the error rates are based on the SVM model after the optimal features 
# for exactly this training set have been selected in rfe.  
...
################################################################################################
set.seed(1714)
index.GA.2 <- createMultiFolds(labels.train.2, k=10, times = 5)        # 10CVn5 crossvalidation 
...
for (i in c(2,3,6,7,8,10)) { Results[i,6] <- length(eval(parse(text=paste("GA.",i, sep="")))$optVariables)  }
Results[11,6] <- length(GA.FINAL$optVariables)
...
for (i in c(2,3,6,7,8,10)) { Results[i,9] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[1] }

write.table(Results, file = "AllResults_rfe_GA40_10TestSets_FinalSet152.txt", sep="\t",col.names=NA)
