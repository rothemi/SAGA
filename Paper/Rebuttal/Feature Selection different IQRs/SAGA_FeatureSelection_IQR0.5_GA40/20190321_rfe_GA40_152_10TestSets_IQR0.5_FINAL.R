#############################################################################################################################################
#################################### Construction of SAGA-SVM Classifier IQR 0.5 ############################################################
#############################################################################################################################################
library(limma)
library(genefilter)
library(Rtsne)
...
matrix.train.1 <- eset.batch[, split.1] # training matrix 107 samples x 36226 predictors 
matrix.test.1  <- eset.batch[,-split.1] # test matrix 45 samples x 36226 predictors 
...
summary(fselect.1)                      # 4550 genes selected with interquartile range of log2-int >0.8
...
##  set feature  subsetSizes to test: 1,2,…,40,45,50,60,….,500,1151  = 52
FeatureNumbers <- c(seq(1,40,by=1),45,50,60,70,80,90,100,200,300,400,500)                 

## set all seeds for reproducibility: 52 seeds for each of the 200 resamples + 1 for complete set 
set.seed(123)
...
################################################################################################
matrix.train.rfe.1 <- matrix.train.1[,rfe.1$optVariables]
...
################################################################################################
set.seed(1714)
index.GA.1 <- createMultiFolds(labels.train.1, k=10, times = 5)        # 10CVn5 crossvalidation 
...
# This will take 3 hrs on AWS EC2 c5.18xlarge instance with 72 CPUs and 144 Gb of RAM
...
################################################################################################
matrix.train.opt.1     <- t(matrix.train.1[,GA.1$optVariables])           # subset to the optVars found by rfe
matrix.test.opt.1      <- matrix.test.1[GA.1$optVariables,]               # subset to the optVars found by rfe
...
rm(matrix.train.opt.1,matrix.test.opt.1,matrix.opt.1,pData.opt.1,pca.1,matrix.full.1,pca.full.1)
##############################################################################################################
...
matrix.test.rfe.1  <- t(matrix.test.1[rfe.1$optVariables,])
matrix.test.GA.1   <- t(matrix.test.1[GA.1$optVariables,])
matrix.train.GA.1  <- matrix.train.1[,GA.1$optVariables]

# train SVM and predict TestSet01 on 35 optVars found by rfe #################################################
set.seed(721)
svmOpt.rfe.1  <- train(matrix.train.rfe.1,labels.train.1,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.1)
...
Prediction_rfe.1 <- predict(svmOpt.rfe.1,matrix.test.rfe.1, type = "prob")
...
# train SVM and predict TestSet01 on 14 optVars found by rfe-GA ##############################################
set.seed(721)
svmOpt.GA.1  <- train(matrix.train.GA.1,labels.train.1,
                      method = "svmRadial",
                      metric = "Accuracy",
                      tuneLength = 20,
                      trControl = fullCtrl.1)
...
Prediction_GA.1 <- predict(svmOpt.GA.1,matrix.test.GA.1, type = "prob")
...
##### Predict TestSet01 with all predictors ##################################################################
...
#############################################################################################################
#### 7 Performance of optVars Classifier on  TestSet 01 #####################################################
#############################################################################################################
...
roc.GA <- roc(Prediction_GA.1$Class,                    # response vector (factor or character)
...
roc.full<- roc(Prediction_SVM_full.1$Class,                    # response vector (factor or character)
...
rfe.2  # selects 4527 features (all input features)
...
##### 4  PCA on optimal variables TestSet 02 #################################################################
##############################################################################################################
matrix.train.rfe.2     <- t(matrix.train.2[,rfe.2$optVariables]) # subset train matrix on the optimal predictors 
matrix.test.rfe.2      <- matrix.test.2[rfe.2$optVariables,]     # subset test matrix on the optimal predictors 
...
plot(pca.2$x, pch=16, col=pData.opt.2$Design_Color, cex=1.5, asp=1)
legend(2,-1, legend = c("transforming","mock","neutral","TestSet"), col = unique(pData.opt.2$Design_Color), pch=16, bty="n", cex=0.8)


##### 5  Train SVM on optVar FeatureSet.2 and predict independent TestSet 02 #################################
##############################################################################################################

set.seed(721)
svmOpt.2  <- train(t(matrix.train.rfe.2),labels.train.2,
                   method = "svmRadial",
                   metric = "Accuracy",
                   tuneLength = 20,
                   trControl = fullCtrl.2)
...
Prediction_rfe.2 <- predict(svmOpt.2,t(matrix.test.rfe.2), type = "prob")
...
#### 6 Performance of opVars Classifier on test samples TestSet 02 ##########################################
#############################################################################################################

#### 6.1 Confusion matrix  ##################################################################################  
sink("TestSet02_ConfusionMatrix_optVars_rfe.txt", append = TRUE)
...
##############################################################################################################
matrix.train.opt.3     <- t(matrix.train.3[,GA.3$optVariables])
matrix.test.opt.3      <- matrix.test.3[GA.3$optVariables,]
...
# clean up
rm(matrix.train.opt.3,matrix.test.opt.3,matrix.opt.3,pData.opt.3)
...
rfe.5    # 16 optimal predictors found 
...
##### 5  PCA on optimal variables ###########################################################################
#############################################################################################################
matrix.train.opt.6     <- t(matrix.train.6[,GA.6$optVariables])
matrix.test.opt.6      <- matrix.test.6[GA.6$optVariables,]
...
# clean up
rm(matrix.train.opt.6,matrix.test.opt.6,matrix.opt.6,pData.opt.6)
...
##### 5  PCA on optimal variables ############################################################################
##############################################################################################################
matrix.train.opt.7     <- t(matrix.train.7[,rfe.7$optVariables])
matrix.test.opt.7      <- matrix.test.7[rfe.7$optVariables,]
...
##############################################################################################################
# subset matrices on the different optimal predictors 
matrix.train.rfe.7  <- matrix.train.7[,rfe.7$optVariables]
...
rfe.8   # 4727 variables found by SVM-rfe
...
##### 5  PCA on optimal variables ############################################################################
##############################################################################################################
matrix.train.opt.8     <- t(matrix.train.8[,rfe.8$optVariables])
matrix.test.opt.8      <- matrix.test.8[rfe.8$optVariables,]
...
# clean up
rm(matrix.train.opt.8,matrix.test.opt.8,matrix.opt.8,pData.opt.8)
...
# subset matrices on the different optimal predictors 
matrix.train.rfe.8  <- matrix.train.8[,rfe.8$optVariables]
...
rfe.FINAL   # 4633 optVars found by rfe
...
##############################################################################################################
matrix.train.rfe.FINAL <- t(matrix.train.FINAL[,rfe.FINAL$optVariables])
pca.FINAL              <- prcomp(t(matrix.train.rfe.FINAL))           
plot(pca.FINAL$x, pch=16, col=pData$Design_Color, cex=1.5, asp=1)
legend(-50,45, legend = c("transforming","mock","neutral"), col = unique(pData$Design_Color), pch=16, bty="n", cex=1)
...
### 2. Supplementary Table X: Resampling and TestSet accuracy for IQR = 0.5 #################################################################
#############################################################################################################################################
library(gmodels)

### 1. create data.frame filled with NAs
...
for (i in c(1,3,6)) { Results[i,11] <- length(eval(parse(text=paste("GA.",i, sep="")))$optVariables)  }

### 12. Resampling accuracy for the GA model
for (i in c(1,3,6)) { Results[i,12] <- eval(parse(text=paste("accuracy.external.",i, sep=""))) }
...
for (i in c(1,3,6)) {Results[i,13] <- ci(eval(parse(text=paste("accuracy.external.opt.",i, sep=""))))[2]}
...
for (i in c(1,3,6)) {Results[i,14] <- ci(eval(parse(text=paste("accuracy.external.opt.",i, sep=""))))[3]}
...
for (i in c(1,3,6)) { Results[i,21] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[1] }

### 22. lower bound of confidence intervall TestSet accuracy for the SVM-GA model
for (i in c(1,3,6)) { Results[i,22] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[3] }

### 23. upper bound of confidence intervall TestSet accuracy for the SVM-GA model
for (i in c(1,3,6)) { Results[i,23] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[4] }

write.table(Results, file = "AllResults_rfe_GA40_10TestSets_FinalSet152_IQR0.5.txt", sep="\t",col.names=NA)

### 3. Plot resampling results vs TestSet results: ##########################################################################################
#############################################################################################################################################

df.results <- Results[c(1:10),]


### for full model 
Accuracy_TestSet_full.mean   <- mean(df.results$Accuracy_TestSet_full)
ResamplingAccuracy_full.mean <- mean(df.results$ResamplingAccuracy_full)
...
ggplot(df.results, aes(Accuracy_TestSet_full,ResamplingAccuracy_full, label = Testset)) +
  ...
  geom_point(aes(x=Accuracy_TestSet_full.mean, y=ResamplingAccuracy_full.mean), colour="red", size = 6)+
  coord_fixed(ratio = 1, xlim=c(0.7,1),ylim=c(0.7,1)) +
  geom_abline(intercept = 0, slope = 1, color = "red",linetype = "dashed") + 
  geom_text(aes(label=Testset),hjust=0.5, vjust=0.5, colour = "white") +
  scale_y_continuous(breaks=seq(0.7,1, 0.02)) + 
  scale_x_continuous(breaks=seq(0.7,1, 0.02)) + 
...
### for SVM-rfe 
Accuracy_TestSet_rfe.mean   <- mean(df.results$Accuracy_TestSet_rfe)
ResamplingAccuracy_rfe.mean <- mean(df.results$ResamplingAccuracy_rfe)
...
ggplot(df.results, aes(Accuracy_TestSet_rfe,ResamplingAccuracy_rfe, label = Testset)) +
  ...
  geom_point(aes(x=Accuracy_TestSet_rfe.mean, y=ResamplingAccuracy_rfe.mean), colour="red", size = 6)+
  coord_fixed(ratio = 1, xlim=c(0.7,1),ylim=c(0.7,1)) +
  geom_abline(intercept = 0, slope = 1, color = "red",linetype = "dashed") + 
  geom_text(aes(label=Testset),hjust=0.5, vjust=0.5, colour = "white") +
  scale_y_continuous(breaks=seq(0.7,1, 0.02)) + 
...
### for SVM-GA
df.results.GA              <- df.results[c(1,3,6),]    # subset for the Training / TestSet splits in which GA was performed 
Accuracy_TestSet_GA.mean   <- mean(df.results.GA$Accuracy_TestSet_GA)
ResamplingAccuracy_GA.mean <- mean(df.results.GA$ResamplingAccuracy_GA)
...
ggplot(df.results.GA, aes(Accuracy_TestSet_GA,ResamplingAccuracy_GA, label = Testset)) +
  ...
  geom_point(aes(x=Accuracy_TestSet_GA.mean, y=ResamplingAccuracy_GA.mean), colour="red", size = 6)+
  coord_fixed(ratio = 1, xlim=c(0.7,1),ylim=c(0.7,1)) +
  geom_abline(intercept = 0, slope = 1, color = "red",linetype = "dashed") + 
  geom_text(aes(label=Testset),hjust=0.5, vjust=0.5, colour = "white") +
  scale_y_continuous(breaks=seq(0.7,1, 0.02)) + 
