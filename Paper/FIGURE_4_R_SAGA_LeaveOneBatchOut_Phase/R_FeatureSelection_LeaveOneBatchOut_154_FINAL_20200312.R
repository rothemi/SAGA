...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 01 #####################
##############################################################################################################
library(bapred)
## the bapred package (Hornung et al 2016) is used to perform AddOn Normalization and batch-correction of the Training and TestSet separately
## which prevents modification of the training set (data leakage) by adding the test samples
## we start from the unmodified EListRaw object, which habors the unmodified RAW intensities of all samples 

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.1    <- qunormtrain(t(RAW_train.1$E))                                # quantile-normalize training samples only
matrix.train.1.qn <- log2(t(qunorm.train.1$xnorm))                                # extract quantile-normalized training samples
matrix.train.1.qn <- avereps(matrix.train.1.qn, ID= RAW_train.1$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.1.qn) <- row.names(pData.train.1)

boxplot(matrix.train.1.qn,col=pData.train.1$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.1.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.1$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.1         <- as.factor(ifelse(pData.train.1$Batch>1,pData.train.1$Batch-1,pData.train.1$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.1        <- combatba(t(matrix.train.1.qn), batch = batch.train.1)                                  # batch training samples only
matrix.train.1.bapred <- t(combat.train.1$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.1.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.1$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.1.bapred.GA   <- t(matrix.train.1.bapred[GA.1$optVariables,])
matrix.train.1.bapred.rfe  <- t(matrix.train.1.bapred[rfe.1$optVariables,])
matrix.train.1.bapred.full <- t(matrix.train.1.bapred[colnames(matrix.train.1),])
...
svmOpt.GA.1  <- train(matrix.train.1.bapred.GA,labels.train.1,
                      ...
                      tuneLength = 20,
                      trControl = fullCtrl.1)
svmOpt.GA.1  # best CV accuracy = 0.955


set.seed(721)
svmOpt.rfe.1  <- train(matrix.train.1.bapred.rfe,labels.train.1,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.1)
svmOpt.rfe.1  # best CV accuracy = 0.942


set.seed(721)
svmOpt.full.1  <- train(matrix.train.1.bapred.full,labels.train.1,
                      method = "svmRadial",
                      metric = "Accuracy",
                      tuneLength = 20,
                      trControl = fullCtrl.1)
svmOpt.full.1  # best CV accuracy = 0.91
...
##############################################################################################################
matrix.test.1.qn  <- log2(t(qunormaddon(qunorm.train.1, t(RAW_test.1$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.1.qn  <- avereps(matrix.test.1.qn, ID= RAW_test.1$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.1.qn,matrix.test.1.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.1$IVIM_Color,pData.test.1$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.1.bapred  <- t(combatbaaddon(combat.train.1, t(matrix.test.1.qn), batch = as.factor(pData.test.1$Batch))) # addon batch-correct test samples / training set is fixed
...
matrix.test.1.bapred.rfe  <- t(matrix.test.1.bapred[rfe.1$optVariables,])
matrix.test.1.bapred.full <- t(matrix.test.1.bapred[colnames(matrix.train.1),])

#### 7.7 Figure 4b and 4c: project test samples into optimal predictor PCA plot spanned by training set ######
##############################################################################################################
pca.train <- prcomp(matrix.train.1.bapred.GA, center = T, scale. = T)           
plot(pca.train$x, pch=16, col=c(pData.train.1$Design_Color), cex=1.8, asp=1)  # Figure 4b
legend(-4,3, legend=unique(pData.train.1$Design), col=unique(pData.train.1$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.1.bapred.GA) # Figure 4c
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.1$Design_Color,pData.test.1$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.1 <- predict(svmOpt.full.1, matrix.test.1.bapred.full, type = "prob")
...
Prediction_rfe.1 <- predict(svmOpt.rfe.1,matrix.test.1.bapred.rfe, type = "prob")
...
write.table(Prediction_rfe.1, file = paste("TestSet01_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 01 #####################################################
#############################################################################################################

sink("TestSet01_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.1$Prediction_rfe.1), as.factor(Prediction_rfe.1$TrueLabel))
sink()

sink("TestSet01_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 01 ########################################################

Prediction_rfe.1$Class <- as.factor(ifelse(Prediction_rfe.1$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.1 <- roc(Prediction_rfe.1$Class,                    
               Prediction_rfe.1$transforming,             
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 01 ####################################
Prediction_rfe.1$Class_Code <- ifelse(Prediction_rfe.1$TrueLabel == "transforming",1,0)
pr.rfe.1 <- pr.curve(scores.class0 = Prediction_rfe.1$transforming , weights.class0 = Prediction_rfe.1$Class_Code, curve = TRUE,rand.compute = T)
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 02 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.2    <- qunormtrain(t(RAW_train.2$E))                                # quantile-normalize training samples only
matrix.train.2.qn <- log2(t(qunorm.train.2$xnorm))                                # extract quantile-normalized training samples
matrix.train.2.qn <- avereps(matrix.train.2.qn, ID= RAW_train.2$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.2.qn) <- row.names(pData.train.2)

boxplot(matrix.train.2.qn,col=pData.train.2$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.2.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.2$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.2         <- as.factor(ifelse(pData.train.2$Batch>2,pData.train.2$Batch-1,pData.train.2$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.2        <- combatba(t(matrix.train.2.qn), batch = batch.train.2)                                  # batch training samples only
matrix.train.2.bapred <- t(combat.train.2$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.2.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.2$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.2.bapred.rfe  <- t(matrix.train.2.bapred[rfe.2$optVariables,])
matrix.train.2.bapred.full <- t(matrix.train.2.bapred[colnames(matrix.train.2),])
...
set.seed(721)
svmOpt.full.2  <- train(matrix.train.2.bapred.full,labels.train.2,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.2)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.2.qn  <- log2(t(qunormaddon(qunorm.train.2, t(RAW_test.2$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.2.qn  <- avereps(matrix.test.2.qn, ID= RAW_test.2$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.2.qn,matrix.test.2.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.2$IVIM_Color,pData.test.2$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.2.bapred  <- t(combatbaaddon(combat.train.2, t(matrix.test.2.qn), batch = as.factor(rep(1,length(row.names(pData.test.2))))))
...
legend(-4.2,4.2, legend=unique(pData.train.2$Design), col=unique(pData.train.2$Design_Color), pch=16, bty="n", cex=1)

coord.pca.test <- predict(pca.train, newdata = matrix.test.2.bapred.rfe)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.2$Design_Color,pData.test.2$IVIM_Color), cex=1.8, asp=1)
text(coord.pca.test, labels=row.names(pData.test.2), cex= 0.4, pos=3, offset = 0.3) 

#### 7.8 predict add-on adjusted test samples  ###############################################################
##############################################################################################################

Prediction_SVM_full.2 <- predict(svmOpt.full.2, matrix.test.2.bapred.full, type = "prob")
...
#### 7.10 Performance of optVars Classifier on  TestSet 02 #####################################################
#############################################################################################################
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 02 ####################################
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 03 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.3    <- qunormtrain(t(RAW_train.3$E))                                # quantile-normalize training samples only
matrix.train.3.qn <- log2(t(qunorm.train.3$xnorm))                                # extract quantile-normalized training samples
matrix.train.3.qn <- avereps(matrix.train.3.qn, ID= RAW_train.3$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.3.qn) <- row.names(pData.train.3)

boxplot(matrix.train.3.qn,col=pData.train.3$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.3.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.3$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.3         <- as.factor(ifelse(pData.train.3$Batch>3,pData.train.3$Batch-1,pData.train.3$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.3        <- combatba(t(matrix.train.3.qn), batch = batch.train.3)                                  # batch training samples only
matrix.train.3.bapred <- t(combat.train.3$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.3.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.3$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.3.bapred.GA   <- t(matrix.train.3.bapred[GA.3$optVariables,])
matrix.train.3.bapred.rfe  <- t(matrix.train.3.bapred[rfe.3$optVariables,])
matrix.train.3.bapred.full <- t(matrix.train.3.bapred[colnames(matrix.train.3),])
...
set.seed(721)
svmOpt.rfe.3  <- train(matrix.train.3.bapred.rfe,labels.train.3,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.3)
...
set.seed(721)
svmOpt.full.3  <- train(matrix.train.3.bapred.full,labels.train.3,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.3)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.3.qn  <- log2(t(qunormaddon(qunorm.train.3, t(RAW_test.3$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.3.qn  <- avereps(matrix.test.3.qn, ID= RAW_test.3$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.3.qn,matrix.test.3.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.3$IVIM_Color,pData.test.3$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.3.bapred  <- t(combatbaaddon(combat.train.3, t(matrix.test.3.qn), batch = as.factor(rep(1,length(row.names(pData.test.3))))))
...
matrix.test.3.bapred.rfe  <- t(matrix.test.3.bapred[rfe.3$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.3$Design), col=unique(pData.train.3$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.3.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.3$Design_Color,pData.test.3$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.3 <- predict(svmOpt.full.3, matrix.test.3.bapred.full, type = "prob")
...
Prediction_rfe.3 <- predict(svmOpt.rfe.3,matrix.test.3.bapred.rfe, type = "prob")
...
#### 7.10 Performance of optVars Classifier on  TestSet 03 #####################################################
#############################################################################################################
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 03 ####################################
Prediction_rfe.3$Class_Code <- ifelse(Prediction_rfe.3$TrueLabel == "transforming",1,0)
pr.rfe.3 <- pr.curve(scores.class0 = Prediction_rfe.3$transforming , weights.class0 = Prediction_rfe.3$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.3,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 04 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.4    <- qunormtrain(t(RAW_train.4$E))                                # quantile-normalize training samples only
matrix.train.4.qn <- log2(t(qunorm.train.4$xnorm))                                # extract quantile-normalized training samples
matrix.train.4.qn <- avereps(matrix.train.4.qn, ID= RAW_train.4$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.4.qn) <- row.names(pData.train.4)

boxplot(matrix.train.4.qn,col=pData.train.4$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.4.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.4$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.4         <- as.factor(ifelse(pData.train.4$Batch>4,pData.train.4$Batch-1,pData.train.4$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.4        <- combatba(t(matrix.train.4.qn), batch = batch.train.4)                                  # batch training samples only
matrix.train.4.bapred <- t(combat.train.4$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.4.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.4$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.4.bapred.GA   <- t(matrix.train.4.bapred[GA.4$optVariables,])
matrix.train.4.bapred.rfe  <- t(matrix.train.4.bapred[rfe.4$optVariables,])
matrix.train.4.bapred.full <- t(matrix.train.4.bapred[colnames(matrix.train.4),])
...
set.seed(721)
svmOpt.rfe.4  <- train(matrix.train.4.bapred.rfe,labels.train.4,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.4)
...
set.seed(721)
svmOpt.full.4  <- train(matrix.train.4.bapred.full,labels.train.4,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.4)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.4.qn  <- log2(t(qunormaddon(qunorm.train.4, t(RAW_test.4$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.4.qn  <- avereps(matrix.test.4.qn, ID= RAW_test.4$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.4.qn,matrix.test.4.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.4$IVIM_Color,pData.test.4$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.4.bapred  <- t(combatbaaddon(combat.train.4, t(matrix.test.4.qn), batch = as.factor(rep(1,length(row.names(pData.test.4))))))
...
matrix.test.4.bapred.rfe  <- t(matrix.test.4.bapred[rfe.4$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.4$Design), col=unique(pData.train.4$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.4.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.4$Design_Color,pData.test.4$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.4 <- predict(svmOpt.full.4, matrix.test.4.bapred.full, type = "prob")
...
Prediction_rfe.4 <- predict(svmOpt.rfe.4,matrix.test.4.bapred.rfe, type = "prob")
...
#### 7.10 Performance of optVars Classifier on  TestSet 04 #####################################################
#############################################################################################################

sink("TestSet04_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.4$Prediction_rfe.4), as.factor(Prediction_rfe.4$TrueLabel))
sink()

sink("TestSet04_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 04 ####################################
Prediction_rfe.4$Class_Code <- ifelse(Prediction_rfe.4$TrueLabel == "transforming",1,0)
pr.rfe.4 <- pr.curve(scores.class0 = Prediction_rfe.4$transforming , weights.class0 = Prediction_rfe.4$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.4,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 05 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.5    <- qunormtrain(t(RAW_train.5$E))                                # quantile-normalize training samples only
matrix.train.5.qn <- log2(t(qunorm.train.5$xnorm))                                # extract quantile-normalized training samples
matrix.train.5.qn <- avereps(matrix.train.5.qn, ID= RAW_train.5$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.5.qn) <- row.names(pData.train.5)

boxplot(matrix.train.5.qn,col=pData.train.5$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.5.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.5$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.5         <- as.factor(ifelse(pData.train.5$Batch>5,pData.train.5$Batch-1,pData.train.5$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.5        <- combatba(t(matrix.train.5.qn), batch = batch.train.5)                                  # batch training samples only
matrix.train.5.bapred <- t(combat.train.5$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.5.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.5$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.5.bapred.GA   <- t(matrix.train.5.bapred[GA.5$optVariables,])
matrix.train.5.bapred.rfe  <- t(matrix.train.5.bapred[rfe.5$optVariables,])
matrix.train.5.bapred.full <- t(matrix.train.5.bapred[colnames(matrix.train.5),])
...
set.seed(721)
svmOpt.rfe.5  <- train(matrix.train.5.bapred.rfe,labels.train.5,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.5)
...
set.seed(721)
svmOpt.full.5  <- train(matrix.train.5.bapred.full,labels.train.5,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.5)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.5.qn  <- log2(t(qunormaddon(qunorm.train.5, t(RAW_test.5$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.5.qn  <- avereps(matrix.test.5.qn, ID= RAW_test.5$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.5.qn,matrix.test.5.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.5$IVIM_Color,pData.test.5$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.5.bapred  <- t(combatbaaddon(combat.train.5, t(matrix.test.5.qn), batch = as.factor(rep(1,length(row.names(pData.test.5))))))
...
matrix.test.5.bapred.rfe  <- t(matrix.test.5.bapred[rfe.5$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.5$Design), col=unique(pData.train.5$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.5.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.5$Design_Color,pData.test.5$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.5 <- predict(svmOpt.full.5, matrix.test.5.bapred.full, type = "prob")
...
Prediction_rfe.5 <- predict(svmOpt.rfe.5,matrix.test.5.bapred.rfe, type = "prob")
...
#### 7.10 Performance of optVars Classifier on  TestSet 05 #####################################################
#############################################################################################################

sink("TestSet05_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.5$Prediction_rfe.5), as.factor(Prediction_rfe.5$TrueLabel))
sink()

sink("TestSet05_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 05 ########################################################

Prediction_rfe.5$Class <- as.factor(ifelse(Prediction_rfe.5$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.5 <- roc(Prediction_rfe.5$Class,                    
                 Prediction_rfe.5$transforming,             
                 percent=TRUE, levels=c("nontransforming","transforming"),
                 plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                 print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 05 ####################################
Prediction_rfe.5$Class_Code <- ifelse(Prediction_rfe.5$TrueLabel == "transforming",1,0)
pr.rfe.5 <- pr.curve(scores.class0 = Prediction_rfe.5$transforming , weights.class0 = Prediction_rfe.5$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.5,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 06 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.6    <- qunormtrain(t(RAW_train.6$E))                                # quantile-normalize training samples only
matrix.train.6.qn <- log2(t(qunorm.train.6$xnorm))                                # extract quantile-normalized training samples
matrix.train.6.qn <- avereps(matrix.train.6.qn, ID= RAW_train.6$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.6.qn) <- row.names(pData.train.6)

boxplot(matrix.train.6.qn,col=pData.train.6$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.6.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.6$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.6         <- as.factor(ifelse(pData.train.6$Batch>6,pData.train.6$Batch-1,pData.train.6$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.6        <- combatba(t(matrix.train.6.qn), batch = batch.train.6)                                  # batch training samples only
matrix.train.6.bapred <- t(combat.train.6$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.6.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.6$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.6.bapred.GA   <- t(matrix.train.6.bapred[GA.6$optVariables,])
matrix.train.6.bapred.rfe  <- t(matrix.train.6.bapred[rfe.6$optVariables,])
matrix.train.6.bapred.full <- t(matrix.train.6.bapred[colnames(matrix.train.6),])
...
set.seed(721)
svmOpt.rfe.6  <- train(matrix.train.6.bapred.rfe,labels.train.6,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.6)
...
set.seed(721)
svmOpt.full.6  <- train(matrix.train.6.bapred.full,labels.train.6,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.6)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.6.qn  <- log2(t(qunormaddon(qunorm.train.6, t(RAW_test.6$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.6.qn  <- avereps(matrix.test.6.qn, ID= RAW_test.6$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.6.qn,matrix.test.6.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.6$IVIM_Color,pData.test.6$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.6.bapred  <- t(combatbaaddon(combat.train.6, t(matrix.test.6.qn), batch = as.factor(rep(1,length(row.names(pData.test.6))))))
...
matrix.test.6.bapred.rfe  <- t(matrix.test.6.bapred[rfe.6$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.6$Design), col=unique(pData.train.6$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.6.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.6$Design_Color,pData.test.6$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.6 <- predict(svmOpt.full.6, matrix.test.6.bapred.full, type = "prob")
...
Prediction_rfe.6 <- predict(svmOpt.rfe.6,matrix.test.6.bapred.rfe, type = "prob")
...
#### 7.10 Performance of optVars Classifier on  TestSet 06 #####################################################
#############################################################################################################
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 06 ####################################
Prediction_rfe.6$Class_Code <- ifelse(Prediction_rfe.6$TrueLabel == "transforming",1,0)
pr.rfe.6 <- pr.curve(scores.class0 = Prediction_rfe.6$transforming , weights.class0 = Prediction_rfe.6$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.6,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 07 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.7    <- qunormtrain(t(RAW_train.7$E))                                # quantile-normalize training samples only
matrix.train.7.qn <- log2(t(qunorm.train.7$xnorm))                                # extract quantile-normalized training samples
matrix.train.7.qn <- avereps(matrix.train.7.qn, ID= RAW_train.7$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.7.qn) <- row.names(pData.train.7)

boxplot(matrix.train.7.qn,col=pData.train.7$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.7.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.7$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.7         <- as.factor(ifelse(pData.train.7$Batch>7,pData.train.7$Batch-1,pData.train.7$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.7        <- combatba(t(matrix.train.7.qn), batch = batch.train.7)                                  # batch training samples only
matrix.train.7.bapred <- t(combat.train.7$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.7.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.7$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.7.bapred.GA   <- t(matrix.train.7.bapred[GA.7$optVariables,])
matrix.train.7.bapred.rfe  <- t(matrix.train.7.bapred[rfe.7$optVariables,])
matrix.train.7.bapred.full <- t(matrix.train.7.bapred[colnames(matrix.train.7),])
...
set.seed(721)
svmOpt.rfe.7  <- train(matrix.train.7.bapred.rfe,labels.train.7,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.7)
...
set.seed(721)
svmOpt.full.7  <- train(matrix.train.7.bapred.full,labels.train.7,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.7)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.7.qn  <- log2(t(qunormaddon(qunorm.train.7, t(RAW_test.7$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.7.qn  <- avereps(matrix.test.7.qn, ID= RAW_test.7$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.7.qn,matrix.test.7.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.7$IVIM_Color,pData.test.7$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.7.bapred  <- t(combatbaaddon(combat.train.7, t(matrix.test.7.qn), batch = as.factor(rep(1,length(row.names(pData.test.7))))))
...
matrix.test.7.bapred.rfe  <- t(matrix.test.7.bapred[rfe.7$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.7$Design), col=unique(pData.train.7$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.7.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.7$Design_Color,pData.test.7$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.7 <- predict(svmOpt.full.7, matrix.test.7.bapred.full, type = "prob")
...
Prediction_rfe.7 <- predict(svmOpt.rfe.7,matrix.test.7.bapred.rfe, type = "prob")
...
#### 7.10 Performance of optVars Classifier on  TestSet 07 #####################################################
#############################################################################################################
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 07 ####################################
Prediction_rfe.7$Class_Code <- ifelse(Prediction_rfe.7$TrueLabel == "transforming",1,0)
pr.rfe.7 <- pr.curve(scores.class0 = Prediction_rfe.7$transforming , weights.class0 = Prediction_rfe.7$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.7,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 08 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.8    <- qunormtrain(t(RAW_train.8$E))                                # quantile-normalize training samples only
matrix.train.8.qn <- log2(t(qunorm.train.8$xnorm))                                # extract quantile-normalized training samples
matrix.train.8.qn <- avereps(matrix.train.8.qn, ID= RAW_train.8$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.8.qn) <- row.names(pData.train.8)

boxplot(matrix.train.8.qn,col=pData.train.8$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.8.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.8$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.8         <- as.factor(ifelse(pData.train.8$Batch>8,pData.train.8$Batch-1,pData.train.8$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.8        <- combatba(t(matrix.train.8.qn), batch = batch.train.8)                                  # batch training samples only
matrix.train.8.bapred <- t(combat.train.8$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.8.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.8$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.8.bapred.GA   <- t(matrix.train.8.bapred[GA.8$optVariables,])
matrix.train.8.bapred.rfe  <- t(matrix.train.8.bapred[rfe.8$optVariables,])
matrix.train.8.bapred.full <- t(matrix.train.8.bapred[colnames(matrix.train.8),])
...
set.seed(721)
svmOpt.rfe.8  <- train(matrix.train.8.bapred.rfe,labels.train.8,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.8)
...
set.seed(721)
svmOpt.full.8  <- train(matrix.train.8.bapred.full,labels.train.8,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.8)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.8.qn  <- log2(t(qunormaddon(qunorm.train.8, t(RAW_test.8$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.8.qn  <- avereps(matrix.test.8.qn, ID= RAW_test.8$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.8.qn,matrix.test.8.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.8$IVIM_Color,pData.test.8$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.8.bapred  <- t(combatbaaddon(combat.train.8, t(matrix.test.8.qn), batch = as.factor(rep(1,length(row.names(pData.test.8))))))
...
matrix.test.8.bapred.rfe  <- t(matrix.test.8.bapred[rfe.8$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.8$Design), col=unique(pData.train.8$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.8.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.8$Design_Color,pData.test.8$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.8 <- predict(svmOpt.full.8, matrix.test.8.bapred.full, type = "prob")
...
Prediction_rfe.8 <- predict(svmOpt.rfe.8,matrix.test.8.bapred.rfe, type = "prob")
...
#### 7.10 Performance of optVars Classifier on  TestSet 08 #####################################################
#############################################################################################################
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 08 ####################################
Prediction_rfe.8$Class_Code <- ifelse(Prediction_rfe.8$TrueLabel == "transforming",1,0)
pr.rfe.8 <- pr.curve(scores.class0 = Prediction_rfe.8$transforming , weights.class0 = Prediction_rfe.8$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.8,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 09 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.9    <- qunormtrain(t(RAW_train.9$E))                                # quantile-normalize training samples only
matrix.train.9.qn <- log2(t(qunorm.train.9$xnorm))                                # extract quantile-normalized training samples
matrix.train.9.qn <- avereps(matrix.train.9.qn, ID= RAW_train.9$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.9.qn) <- row.names(pData.train.9)

boxplot(matrix.train.9.qn,col=pData.train.9$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.9.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.9$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.9         <- as.factor(ifelse(pData.train.9$Batch>9,pData.train.9$Batch-1,pData.train.9$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.9        <- combatba(t(matrix.train.9.qn), batch = batch.train.9)                                  # batch training samples only
matrix.train.9.bapred <- t(combat.train.9$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.9.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.9$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.9.bapred.GA   <- t(matrix.train.9.bapred[GA.9$optVariables,])
matrix.train.9.bapred.rfe  <- t(matrix.train.9.bapred[rfe.9$optVariables,])
matrix.train.9.bapred.full <- t(matrix.train.9.bapred[colnames(matrix.train.9),])
...
set.seed(721)
svmOpt.rfe.9  <- train(matrix.train.9.bapred.rfe,labels.train.9,
                       method = "svmRadial",
                       metric = "Accuracy",
                       tuneLength = 20,
                       trControl = fullCtrl.9)
...
set.seed(721)
svmOpt.full.9  <- train(matrix.train.9.bapred.full,labels.train.9,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.9)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.9.qn  <- log2(t(qunormaddon(qunorm.train.9, t(RAW_test.9$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.9.qn  <- avereps(matrix.test.9.qn, ID= RAW_test.9$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.9.qn,matrix.test.9.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.9$IVIM_Color,pData.test.9$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.9.bapred  <- t(combatbaaddon(combat.train.9, t(matrix.test.9.qn), batch = as.factor(rep(1,length(row.names(pData.test.9))))))
...
matrix.test.9.bapred.rfe  <- t(matrix.test.9.bapred[rfe.9$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.9$Design), col=unique(pData.train.9$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.9.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.9$Design_Color,pData.test.9$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.9 <- predict(svmOpt.full.9, matrix.test.9.bapred.full, type = "prob")
...
Prediction_rfe.9 <- predict(svmOpt.rfe.9,matrix.test.9.bapred.rfe, type = "prob")
...
#### 7.10 Performance of optVars Classifier on  TestSet 09 #####################################################
#############################################################################################################

sink("TestSet09_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.9$Prediction_rfe.9), as.factor(Prediction_rfe.9$TrueLabel))
sink()

sink("TestSet09_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 09 ####################################
Prediction_rfe.9$Class_Code <- ifelse(Prediction_rfe.9$TrueLabel == "transforming",1,0)
pr.rfe.9 <- pr.curve(scores.class0 = Prediction_rfe.9$transforming , weights.class0 = Prediction_rfe.9$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.9,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 010 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.10    <- qunormtrain(t(RAW_train.10$E))                                # quantile-normalize training samples only
matrix.train.10.qn <- log2(t(qunorm.train.10$xnorm))                                # extract quantile-normalized training samples
matrix.train.10.qn <- avereps(matrix.train.10.qn, ID= RAW_train.10$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.10.qn) <- row.names(pData.train.10)

boxplot(matrix.train.10.qn,col=pData.train.10$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.10.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.10$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.10         <- as.factor(ifelse(pData.train.10$Batch>10,pData.train.10$Batch-1,pData.train.10$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.10        <- combatba(t(matrix.train.10.qn), batch = batch.train.10)                                  # batch training samples only
matrix.train.10.bapred <- t(combat.train.10$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.10.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.10$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.10.bapred.GA   <- t(matrix.train.10.bapred[GA.10$optVariables,])
matrix.train.10.bapred.rfe  <- t(matrix.train.10.bapred[rfe.10$optVariables,])
matrix.train.10.bapred.full <- t(matrix.train.10.bapred[colnames(matrix.train.10),])
...
set.seed(721)
svmOpt.rfe.10  <- train(matrix.train.10.bapred.rfe,labels.train.10,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.10)
...
set.seed(721)
svmOpt.full.10  <- train(matrix.train.10.bapred.full,labels.train.10,
                         method = "svmRadial",
                         metric = "Accuracy",
                         tuneLength = 20,
                         trControl = fullCtrl.10)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.10.qn  <- log2(t(qunormaddon(qunorm.train.10, t(RAW_test.10$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.10.qn  <- avereps(matrix.test.10.qn, ID= RAW_test.10$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.10.qn,matrix.test.10.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.10$IVIM_Color,pData.test.10$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.10.bapred  <- t(combatbaaddon(combat.train.10, t(matrix.test.10.qn), batch = as.factor(rep(1,length(row.names(pData.test.10))))))
...
matrix.test.10.bapred.rfe  <- t(matrix.test.10.bapred[rfe.10$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.10$Design), col=unique(pData.train.10$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.10.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.10$Design_Color,pData.test.10$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.10 <- predict(svmOpt.full.10, matrix.test.10.bapred.full, type = "prob")
...
Prediction_rfe.10 <- predict(svmOpt.rfe.10,matrix.test.10.bapred.rfe, type = "prob")
...
write.table(Prediction_rfe.10, file = paste("TestSet010_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 010 #####################################################
#############################################################################################################

sink("TestSet010_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.10$Prediction_rfe.10), as.factor(Prediction_rfe.10$TrueLabel))
sink()

sink("TestSet010_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 010 ####################################
Prediction_rfe.10$Class_Code <- ifelse(Prediction_rfe.10$TrueLabel == "transforming",1,0)
pr.rfe.10 <- pr.curve(scores.class0 = Prediction_rfe.10$transforming , weights.class0 = Prediction_rfe.10$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.10,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 012 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.12    <- qunormtrain(t(RAW_train.12$E))                                # quantile-normalize training samples only
matrix.train.12.qn <- log2(t(qunorm.train.12$xnorm))                                # extract quantile-normalized training samples
matrix.train.12.qn <- avereps(matrix.train.12.qn, ID= RAW_train.12$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.12.qn) <- row.names(pData.train.12)

boxplot(matrix.train.12.qn,col=pData.train.12$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.12.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.12$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.12         <- as.factor(ifelse(pData.train.12$Batch>12,pData.train.12$Batch-1,pData.train.12$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.12        <- combatba(t(matrix.train.12.qn), batch = batch.train.12)                                  # batch training samples only
matrix.train.12.bapred <- t(combat.train.12$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.12.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.12$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.12.bapred.GA   <- t(matrix.train.12.bapred[GA.12$optVariables,])
matrix.train.12.bapred.rfe  <- t(matrix.train.12.bapred[rfe.12$optVariables,])
matrix.train.12.bapred.full <- t(matrix.train.12.bapred[colnames(matrix.train.12),])
...
set.seed(721)
svmOpt.rfe.12  <- train(matrix.train.12.bapred.rfe,labels.train.12,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.12)
...
set.seed(721)
svmOpt.full.12  <- train(matrix.train.12.bapred.full,labels.train.12,
                         method = "svmRadial",
                         metric = "Accuracy",
                         tuneLength = 20,
                         trControl = fullCtrl.12)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.12.qn  <- log2(t(qunormaddon(qunorm.train.12, t(RAW_test.12$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.12.qn  <- avereps(matrix.test.12.qn, ID= RAW_test.12$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.12.qn,matrix.test.12.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.12$IVIM_Color,pData.test.12$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.12.bapred  <- t(combatbaaddon(combat.train.12, t(matrix.test.12.qn), batch = as.factor(rep(1,length(row.names(pData.test.12))))))
...
matrix.test.12.bapred.rfe  <- t(matrix.test.12.bapred[rfe.12$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.12$Design), col=unique(pData.train.12$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.12.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.12$Design_Color,pData.test.12$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.12 <- predict(svmOpt.full.12, matrix.test.12.bapred.full, type = "prob")
...
Prediction_rfe.12 <- predict(svmOpt.rfe.12,matrix.test.12.bapred.rfe, type = "prob")
Prediction_rfe.12$Prediction_rfe.12 <- ifelse(Prediction_rfe.12$transforming>0.50,"transforming","untransforming")
Prediction_rfe.12 <- cbind(pData.test.12[,c(1:3)],TrueLabel=pData.test.12$Class,Prediction_rfe.12)
write.table(Prediction_rfe.12, file = paste("TestSet012_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 012 #####################################################
#############################################################################################################

sink("TestSet012_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.12$Prediction_rfe.12), as.factor(Prediction_rfe.12$TrueLabel))
sink()

sink("TestSet012_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 012 ########################################################

Prediction_rfe.12$Class <- as.factor(ifelse(Prediction_rfe.12$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.12 <- roc(Prediction_rfe.12$Class,                    
                  Prediction_rfe.12$transforming,             
                  percent=TRUE, levels=c("nontransforming","transforming"),
                  plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                  print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 012 ####################################
Prediction_rfe.12$Class_Code <- ifelse(Prediction_rfe.12$TrueLabel == "transforming",1,0)
pr.rfe.12 <- pr.curve(scores.class0 = Prediction_rfe.12$transforming , weights.class0 = Prediction_rfe.12$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.12,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 013 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.13    <- qunormtrain(t(RAW_train.13$E))                                # quantile-normalize training samples only
matrix.train.13.qn <- log2(t(qunorm.train.13$xnorm))                                # extract quantile-normalized training samples
matrix.train.13.qn <- avereps(matrix.train.13.qn, ID= RAW_train.13$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.13.qn) <- row.names(pData.train.13)

boxplot(matrix.train.13.qn,col=pData.train.13$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.13.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.13$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.13         <- as.factor(ifelse(pData.train.13$Batch>13,pData.train.13$Batch-1,pData.train.13$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.13        <- combatba(t(matrix.train.13.qn), batch = batch.train.13)                                  # batch training samples only
matrix.train.13.bapred <- t(combat.train.13$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.13.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.13$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.13.bapred.GA   <- t(matrix.train.13.bapred[GA.13$optVariables,])
matrix.train.13.bapred.rfe  <- t(matrix.train.13.bapred[rfe.13$optVariables,])
matrix.train.13.bapred.full <- t(matrix.train.13.bapred[colnames(matrix.train.13),])
...
set.seed(721)
svmOpt.rfe.13  <- train(matrix.train.13.bapred.rfe,labels.train.13,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.13)
...
set.seed(721)
svmOpt.full.13  <- train(matrix.train.13.bapred.full,labels.train.13,
                         method = "svmRadial",
                         metric = "Accuracy",
                         tuneLength = 20,
                         trControl = fullCtrl.13)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.13.qn  <- log2(t(qunormaddon(qunorm.train.13, t(RAW_test.13$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.13.qn  <- avereps(matrix.test.13.qn, ID= RAW_test.13$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.13.qn,matrix.test.13.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.13$IVIM_Color,pData.test.13$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.13.bapred  <- t(combatbaaddon(combat.train.13, t(matrix.test.13.qn), batch = as.factor(rep(1,length(row.names(pData.test.13))))))
...
matrix.test.13.bapred.rfe  <- t(matrix.test.13.bapred[rfe.13$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.13$Design), col=unique(pData.train.13$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.13.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.13$Design_Color,pData.test.13$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.13 <- predict(svmOpt.full.13, matrix.test.13.bapred.full, type = "prob")
...
Prediction_rfe.13 <- predict(svmOpt.rfe.13,matrix.test.13.bapred.rfe, type = "prob")
Prediction_rfe.13$Prediction_rfe.13 <- ifelse(Prediction_rfe.13$transforming>0.50,"transforming","untransforming")
Prediction_rfe.13 <- cbind(pData.test.13[,c(1:3)],TrueLabel=pData.test.13$Class,Prediction_rfe.13)
write.table(Prediction_rfe.13, file = paste("TestSet013_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 013 #####################################################
#############################################################################################################

sink("TestSet013_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.13$Prediction_rfe.13), as.factor(Prediction_rfe.13$TrueLabel))
sink()

sink("TestSet013_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 013 ########################################################

Prediction_rfe.13$Class <- as.factor(ifelse(Prediction_rfe.13$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.13 <- roc(Prediction_rfe.13$Class,                    
                  Prediction_rfe.13$transforming,             
                  percent=TRUE, levels=c("nontransforming","transforming"),
                  plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                  print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 013 ####################################
Prediction_rfe.13$Class_Code <- ifelse(Prediction_rfe.13$TrueLabel == "transforming",1,0)
pr.rfe.13 <- pr.curve(scores.class0 = Prediction_rfe.13$transforming , weights.class0 = Prediction_rfe.13$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.13,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 014 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.14    <- qunormtrain(t(RAW_train.14$E))                                # quantile-normalize training samples only
matrix.train.14.qn <- log2(t(qunorm.train.14$xnorm))                                # extract quantile-normalized training samples
matrix.train.14.qn <- avereps(matrix.train.14.qn, ID= RAW_train.14$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.14.qn) <- row.names(pData.train.14)

boxplot(matrix.train.14.qn,col=pData.train.14$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.14.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.14$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.14         <- as.factor(ifelse(pData.train.14$Batch>14,pData.train.14$Batch-1,pData.train.14$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.14        <- combatba(t(matrix.train.14.qn), batch = batch.train.14)                                  # batch training samples only
matrix.train.14.bapred <- t(combat.train.14$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.14.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.14$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.14.bapred.GA   <- t(matrix.train.14.bapred[GA.14$optVariables,])
matrix.train.14.bapred.rfe  <- t(matrix.train.14.bapred[rfe.14$optVariables,])
matrix.train.14.bapred.full <- t(matrix.train.14.bapred[colnames(matrix.train.14),])
...
set.seed(721)
svmOpt.rfe.14  <- train(matrix.train.14.bapred.rfe,labels.train.14,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.14)
...
set.seed(721)
svmOpt.full.14  <- train(matrix.train.14.bapred.full,labels.train.14,
                         method = "svmRadial",
                         metric = "Accuracy",
                         tuneLength = 20,
                         trControl = fullCtrl.14)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.14.qn  <- log2(t(qunormaddon(qunorm.train.14, t(RAW_test.14$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.14.qn  <- avereps(matrix.test.14.qn, ID= RAW_test.14$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.14.qn,matrix.test.14.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.14$IVIM_Color,pData.test.14$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.14.bapred  <- t(combatbaaddon(combat.train.14, t(matrix.test.14.qn), batch = as.factor(rep(1,length(row.names(pData.test.14))))))
...
matrix.test.14.bapred.rfe  <- t(matrix.test.14.bapred[rfe.14$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.14$Design), col=unique(pData.train.14$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.14.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.14$Design_Color,pData.test.14$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.14 <- predict(svmOpt.full.14, matrix.test.14.bapred.full, type = "prob")
...
Prediction_rfe.14 <- predict(svmOpt.rfe.14,matrix.test.14.bapred.rfe, type = "prob")
Prediction_rfe.14$Prediction_rfe.14 <- ifelse(Prediction_rfe.14$transforming>0.50,"transforming","untransforming")
Prediction_rfe.14 <- cbind(pData.test.14[,c(1:3)],TrueLabel=pData.test.14$Class,Prediction_rfe.14)
write.table(Prediction_rfe.14, file = paste("TestSet014_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 014 #####################################################
#############################################################################################################

sink("TestSet014_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.14$Prediction_rfe.14), as.factor(Prediction_rfe.14$TrueLabel))
sink()

sink("TestSet014_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 014 ########################################################

Prediction_rfe.14$Class <- as.factor(ifelse(Prediction_rfe.14$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.14 <- roc(Prediction_rfe.14$Class,                    
                  Prediction_rfe.14$transforming,             
                  percent=TRUE, levels=c("nontransforming","transforming"),
                  plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                  print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 014 ####################################
Prediction_rfe.14$Class_Code <- ifelse(Prediction_rfe.14$TrueLabel == "transforming",1,0)
pr.rfe.14 <- pr.curve(scores.class0 = Prediction_rfe.14$transforming , weights.class0 = Prediction_rfe.14$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.14,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 015 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.15    <- qunormtrain(t(RAW_train.15$E))                                # quantile-normalize training samples only
matrix.train.15.qn <- log2(t(qunorm.train.15$xnorm))                                # extract quantile-normalized training samples
matrix.train.15.qn <- avereps(matrix.train.15.qn, ID= RAW_train.15$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.15.qn) <- row.names(pData.train.15)

boxplot(matrix.train.15.qn,col=pData.train.15$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.15.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.15$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.15         <- as.factor(ifelse(pData.train.15$Batch>15,pData.train.15$Batch-1,pData.train.15$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.15        <- combatba(t(matrix.train.15.qn), batch = batch.train.15)                                  # batch training samples only
matrix.train.15.bapred <- t(combat.train.15$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.15.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.15$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.15.bapred.GA   <- t(matrix.train.15.bapred[GA.15$optVariables,])
matrix.train.15.bapred.rfe  <- t(matrix.train.15.bapred[rfe.15$optVariables,])
matrix.train.15.bapred.full <- t(matrix.train.15.bapred[colnames(matrix.train.15),])
...
set.seed(721)
svmOpt.rfe.15  <- train(matrix.train.15.bapred.rfe,labels.train.15,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.15)
...
set.seed(721)
svmOpt.full.15  <- train(matrix.train.15.bapred.full,labels.train.15,
                         method = "svmRadial",
                         metric = "Accuracy",
                         tuneLength = 20,
                         trControl = fullCtrl.15)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.15.qn  <- log2(t(qunormaddon(qunorm.train.15, t(RAW_test.15$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.15.qn  <- avereps(matrix.test.15.qn, ID= RAW_test.15$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.15.qn,matrix.test.15.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.15$IVIM_Color,pData.test.15$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.15.bapred  <- t(combatbaaddon(combat.train.15, t(matrix.test.15.qn), batch = as.factor(rep(1,length(row.names(pData.test.15))))))
...
matrix.test.15.bapred.rfe  <- t(matrix.test.15.bapred[rfe.15$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.15$Design), col=unique(pData.train.15$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.15.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.15$Design_Color,pData.test.15$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.15 <- predict(svmOpt.full.15, matrix.test.15.bapred.full, type = "prob")
...
Prediction_rfe.15 <- predict(svmOpt.rfe.15,matrix.test.15.bapred.rfe, type = "prob")
Prediction_rfe.15$Prediction_rfe.15 <- ifelse(Prediction_rfe.15$transforming>0.50,"transforming","untransforming")
Prediction_rfe.15 <- cbind(pData.test.15[,c(1:3)],TrueLabel=pData.test.15$Class,Prediction_rfe.15)
write.table(Prediction_rfe.15, file = paste("TestSet015_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 015 #####################################################
#############################################################################################################

sink("TestSet015_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.15$Prediction_rfe.15), as.factor(Prediction_rfe.15$TrueLabel))
sink()

sink("TestSet015_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 015 ########################################################

Prediction_rfe.15$Class <- as.factor(ifelse(Prediction_rfe.15$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.15 <- roc(Prediction_rfe.15$Class,                    
                  Prediction_rfe.15$transforming,             
                  percent=TRUE, levels=c("nontransforming","transforming"),
                  plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                  print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 015 ####################################
Prediction_rfe.15$Class_Code <- ifelse(Prediction_rfe.15$TrueLabel == "transforming",1,0)
pr.rfe.15 <- pr.curve(scores.class0 = Prediction_rfe.15$transforming , weights.class0 = Prediction_rfe.15$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.15,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 016 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.16    <- qunormtrain(t(RAW_train.16$E))                                # quantile-normalize training samples only
matrix.train.16.qn <- log2(t(qunorm.train.16$xnorm))                                # extract quantile-normalized training samples
matrix.train.16.qn <- avereps(matrix.train.16.qn, ID= RAW_train.16$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.16.qn) <- row.names(pData.train.16)

boxplot(matrix.train.16.qn,col=pData.train.16$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.16.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.16$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.16         <- as.factor(ifelse(pData.train.16$Batch>16,pData.train.16$Batch-1,pData.train.16$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.16        <- combatba(t(matrix.train.16.qn), batch = batch.train.16)                                  # batch training samples only
matrix.train.16.bapred <- t(combat.train.16$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.16.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.16$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.16.bapred.GA   <- t(matrix.train.16.bapred[GA.16$optVariables,])
matrix.train.16.bapred.rfe  <- t(matrix.train.16.bapred[rfe.16$optVariables,])
matrix.train.16.bapred.full <- t(matrix.train.16.bapred[colnames(matrix.train.16),])
...
set.seed(721)
svmOpt.rfe.16  <- train(matrix.train.16.bapred.rfe,labels.train.16,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.16)
...
set.seed(721)
svmOpt.full.16  <- train(matrix.train.16.bapred.full,labels.train.16,
                         method = "svmRadial",
                         metric = "Accuracy",
                         tuneLength = 20,
                         trControl = fullCtrl.16)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.16.qn  <- log2(t(qunormaddon(qunorm.train.16, t(RAW_test.16$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.16.qn  <- avereps(matrix.test.16.qn, ID= RAW_test.16$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.16.qn,matrix.test.16.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.16$IVIM_Color,pData.test.16$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.16.bapred  <- t(combatbaaddon(combat.train.16, t(matrix.test.16.qn), batch = as.factor(rep(1,length(row.names(pData.test.16))))))
...
matrix.test.16.bapred.rfe  <- t(matrix.test.16.bapred[rfe.16$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.16$Design), col=unique(pData.train.16$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.16.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.16$Design_Color,pData.test.16$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.16 <- predict(svmOpt.full.16, matrix.test.16.bapred.full, type = "prob")
...
Prediction_rfe.16 <- predict(svmOpt.rfe.16,matrix.test.16.bapred.rfe, type = "prob")
Prediction_rfe.16$Prediction_rfe.16 <- ifelse(Prediction_rfe.16$transforming>0.50,"transforming","untransforming")
Prediction_rfe.16 <- cbind(pData.test.16[,c(1:3)],TrueLabel=pData.test.16$Class,Prediction_rfe.16)
write.table(Prediction_rfe.16, file = paste("TestSet016_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 016 #####################################################
#############################################################################################################

sink("TestSet016_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.16$Prediction_rfe.16), as.factor(Prediction_rfe.16$TrueLabel))
sink()

sink("TestSet016_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 016 ########################################################

Prediction_rfe.16$Class <- as.factor(ifelse(Prediction_rfe.16$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.16 <- roc(Prediction_rfe.16$Class,                    
                  Prediction_rfe.16$transforming,             
                  percent=TRUE, levels=c("nontransforming","transforming"),
                  plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                  print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 016 ####################################
Prediction_rfe.16$Class_Code <- ifelse(Prediction_rfe.16$TrueLabel == "transforming",1,0)
pr.rfe.16 <- pr.curve(scores.class0 = Prediction_rfe.16$transforming , weights.class0 = Prediction_rfe.16$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.16,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 017 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.17    <- qunormtrain(t(RAW_train.17$E))                                # quantile-normalize training samples only
matrix.train.17.qn <- log2(t(qunorm.train.17$xnorm))                                # extract quantile-normalized training samples
matrix.train.17.qn <- avereps(matrix.train.17.qn, ID= RAW_train.17$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.17.qn) <- row.names(pData.train.17)

boxplot(matrix.train.17.qn,col=pData.train.17$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.17.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.17$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.17         <- as.factor(ifelse(pData.train.17$Batch>17,pData.train.17$Batch-1,pData.train.17$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.17        <- combatba(t(matrix.train.17.qn), batch = batch.train.17)                                  # batch training samples only
matrix.train.17.bapred <- t(combat.train.17$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.17.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.17$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.17.bapred.GA   <- t(matrix.train.17.bapred[GA.17$optVariables,])
matrix.train.17.bapred.rfe  <- t(matrix.train.17.bapred[row.names(optFeatures.17),])
matrix.train.17.bapred.full <- t(matrix.train.17.bapred[colnames(matrix.train.17),])
...
set.seed(721)
svmOpt.rfe.17  <- train(matrix.train.17.bapred.rfe,labels.train.17,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.17)
...
set.seed(721)
svmOpt.full.17  <- train(matrix.train.17.bapred.full,labels.train.17,
                         method = "svmRadial",
                         metric = "Accuracy",
                         tuneLength = 20,
                         trControl = fullCtrl.17)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.17.qn  <- log2(t(qunormaddon(qunorm.train.17, t(RAW_test.17$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.17.qn  <- avereps(matrix.test.17.qn, ID= RAW_test.17$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.17.qn,matrix.test.17.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.17$IVIM_Color,pData.test.17$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.17.bapred  <- t(combatbaaddon(combat.train.17, t(matrix.test.17.qn), batch = as.factor(rep(1,length(row.names(pData.test.17))))))
...
matrix.test.17.bapred.rfe  <- t(matrix.test.17.bapred[row.names(optFeatures.17),])
...
legend(-4.2,4.2, legend=unique(pData.train.17$Design), col=unique(pData.train.17$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.17.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.17$Design_Color,pData.test.17$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.17 <- predict(svmOpt.full.17, matrix.test.17.bapred.full, type = "prob")
...
Prediction_rfe.17 <- predict(svmOpt.rfe.17,matrix.test.17.bapred.rfe, type = "prob")
Prediction_rfe.17$Prediction_rfe.17 <- ifelse(Prediction_rfe.17$transforming>0.50,"transforming","untransforming")
Prediction_rfe.17 <- cbind(pData.test.17[,c(1:3)],TrueLabel=pData.test.17$Class,Prediction_rfe.17)
write.table(Prediction_rfe.17, file = paste("TestSet017_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 017 #####################################################
#############################################################################################################

sink("TestSet017_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.17$Prediction_rfe.17), as.factor(Prediction_rfe.17$TrueLabel))
sink()

sink("TestSet017_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 017 ########################################################

Prediction_rfe.17$Class <- as.factor(ifelse(Prediction_rfe.17$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.17 <- roc(Prediction_rfe.17$Class,                    
                  Prediction_rfe.17$transforming,             
                  percent=TRUE, levels=c("nontransforming","transforming"),
                  plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                  print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 017 ####################################
Prediction_rfe.17$Class_Code <- ifelse(Prediction_rfe.17$TrueLabel == "transforming",1,0)
pr.rfe.17 <- pr.curve(scores.class0 = Prediction_rfe.17$transforming , weights.class0 = Prediction_rfe.17$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.17,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 018 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.18    <- qunormtrain(t(RAW_train.18$E))                                # quantile-normalize training samples only
matrix.train.18.qn <- log2(t(qunorm.train.18$xnorm))                                # extract quantile-normalized training samples
matrix.train.18.qn <- avereps(matrix.train.18.qn, ID= RAW_train.18$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.18.qn) <- row.names(pData.train.18)

boxplot(matrix.train.18.qn,col=pData.train.18$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.18.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.18$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.18         <- as.factor(ifelse(pData.train.18$Batch>18,pData.train.18$Batch-1,pData.train.18$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.18        <- combatba(t(matrix.train.18.qn), batch = batch.train.18)                                  # batch training samples only
matrix.train.18.bapred <- t(combat.train.18$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.18.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.18$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.18.bapred.GA   <- t(matrix.train.18.bapred[GA.18$optVariables,])
matrix.train.18.bapred.rfe  <- t(matrix.train.18.bapred[rfe.18$optVariables,])
matrix.train.18.bapred.full <- t(matrix.train.18.bapred[colnames(matrix.train.18),])
...
set.seed(721)
svmOpt.rfe.18  <- train(matrix.train.18.bapred.rfe,labels.train.18,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.18)
...
set.seed(721)
svmOpt.full.18  <- train(matrix.train.18.bapred.full,labels.train.18,
                         method = "svmRadial",
                         metric = "Accuracy",
                         tuneLength = 20,
                         trControl = fullCtrl.18)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.18.qn  <- log2(t(qunormaddon(qunorm.train.18, t(RAW_test.18$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.18.qn  <- avereps(matrix.test.18.qn, ID= RAW_test.18$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.18.qn,matrix.test.18.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.18$IVIM_Color,pData.test.18$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.18.bapred  <- t(combatbaaddon(combat.train.18, t(matrix.test.18.qn), batch = as.factor(rep(1,length(row.names(pData.test.18))))))
...
matrix.test.18.bapred.rfe  <- t(matrix.test.18.bapred[rfe.18$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.18$Design), col=unique(pData.train.18$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.18.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.18$Design_Color,pData.test.18$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.18 <- predict(svmOpt.full.18, matrix.test.18.bapred.full, type = "prob")
...
Prediction_rfe.18 <- predict(svmOpt.rfe.18,matrix.test.18.bapred.rfe, type = "prob")
Prediction_rfe.18$Prediction_rfe.18 <- ifelse(Prediction_rfe.18$transforming>0.50,"transforming","untransforming")
Prediction_rfe.18 <- cbind(pData.test.18[,c(1:3)],TrueLabel=pData.test.18$Class,Prediction_rfe.18)
write.table(Prediction_rfe.18, file = paste("TestSet018_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 018 #####################################################
#############################################################################################################

sink("TestSet018_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.18$Prediction_rfe.18), as.factor(Prediction_rfe.18$TrueLabel))
sink()

sink("TestSet018_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 018 ########################################################

Prediction_rfe.18$Class <- as.factor(ifelse(Prediction_rfe.18$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.18 <- roc(Prediction_rfe.18$Class,                    
                  Prediction_rfe.18$transforming,             
                  percent=TRUE, levels=c("nontransforming","transforming"),
                  plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                  print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 018 ####################################
Prediction_rfe.18$Class_Code <- ifelse(Prediction_rfe.18$TrueLabel == "transforming",1,0)
pr.rfe.18 <- pr.curve(scores.class0 = Prediction_rfe.18$transforming , weights.class0 = Prediction_rfe.18$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.18,rand.plot = TRUE, legend = F, color = 1, main = "")
...
##############################################################################################################
##### 7 Re-Train SVM on TrainingSet reduced to optimal predictors and predict TestSet 019 #####################
##############################################################################################################

#### 7.1 Quantile normalization of training set with bapred ################################################## 
##############################################################################################################
qunorm.train.19    <- qunormtrain(t(RAW_train.19$E))                                # quantile-normalize training samples only
matrix.train.19.qn <- log2(t(qunorm.train.19$xnorm))                                # extract quantile-normalized training samples
matrix.train.19.qn <- avereps(matrix.train.19.qn, ID= RAW_train.19$genes$ProbeName)  # average quadruplicates of training samples  
colnames(matrix.train.19.qn) <- row.names(pData.train.19)

boxplot(matrix.train.19.qn,col=pData.train.19$IVIM_Color,boxwex=0.6,cex.axis=0.5,las=2,outline=FALSE)

set.seed(12)
plot(Rtsne(t(matrix.train.19.qn),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.19$IVIM_Color, pch=16, cex=1.3) 

#### 7.2 COMBAT batch correction of training set with bapred ################################################# 
##############################################################################################################
batch.train.19         <- as.factor(ifelse(pData.train.19$Batch>19,pData.train.19$Batch-1,pData.train.19$Batch))     # create batch factor 1...18 w/o Test samples
combat.train.19        <- combatba(t(matrix.train.19.qn), batch = batch.train.19)                                  # batch training samples only
matrix.train.19.bapred <- t(combat.train.19$xadj)                                                                 # extract batch corrected training matrix 

set.seed(12)
plot(Rtsne(t(matrix.train.19.bapred),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,col=pData.train.19$Design_Color, pch=16, cex=1.3) 

#### 7.3 subset training set for the optimal variables #######################################################
##############################################################################################################
matrix.train.19.bapred.GA   <- t(matrix.train.19.bapred[GA.19$optVariables,])
matrix.train.19.bapred.rfe  <- t(matrix.train.19.bapred[rfe.19$optVariables,])
matrix.train.19.bapred.full <- t(matrix.train.19.bapred[colnames(matrix.train.19),])
...
set.seed(721)
svmOpt.rfe.19  <- train(matrix.train.19.bapred.rfe,labels.train.19,
                        method = "svmRadial",
                        metric = "Accuracy",
                        tuneLength = 20,
                        trControl = fullCtrl.19)
...
#### 7.4 Load and Add-on quantile normalization of the test set ##############################################
##############################################################################################################
matrix.test.19.qn  <- log2(t(qunormaddon(qunorm.train.19, t(RAW_test.19$E))))        # use qunormtrain object to normalize test samples / training set is fixed
matrix.test.19.qn  <- avereps(matrix.test.19.qn, ID= RAW_test.19$genes$ProbeName)    # average quadruplicates of test samples
...
set.seed(12)
plot(Rtsne(t(cbind(matrix.train.19.qn,matrix.test.19.qn)),dims = 2, perplexity = 16,theta = 0.5, check_duplicates = FALSE, 
           pca = TRUE, max_iter = 1000,verbose = FALSE, is_distance = FALSE)$Y,
     col=c(pData.train.19$IVIM_Color,pData.test.19$IVIM_Color), pch=16, cex=1.3) 

#### 7.5 Add-on batch correction of the test set #############################################################
##############################################################################################################
matrix.test.19.bapred  <- t(combatbaaddon(combat.train.19, t(matrix.test.19.qn), batch = as.factor(rep(1,length(row.names(pData.test.19))))))
...
matrix.test.19.bapred.rfe  <- t(matrix.test.19.bapred[rfe.19$optVariables,])
...
legend(-4.2,4.2, legend=unique(pData.train.19$Design), col=unique(pData.train.19$Design_Color), pch=16, bty="n", cex=1)
summary(pca.train)

coord.pca.test <- predict(pca.train, newdata = matrix.test.19.bapred.GA)
plot(rbind(pca.train$x,coord.pca.test), pch=16, col=c(pData.train.19$Design_Color,pData.test.19$IVIM_Color), cex=1.8, asp=1)
...
Prediction_SVM_full.19 <- predict(svmOpt.full.19, matrix.test.19.bapred.full, type = "prob")
...
Prediction_rfe.19 <- predict(svmOpt.rfe.19,matrix.test.19.bapred.rfe, type = "prob")
Prediction_rfe.19$Prediction_rfe.19 <- ifelse(Prediction_rfe.19$transforming>0.50,"transforming","untransforming")
Prediction_rfe.19 <- cbind(pData.test.19[,c(1:3)],TrueLabel=pData.test.19$Class,Prediction_rfe.19)
write.table(Prediction_rfe.19, file = paste("TestSet019_Predictions_optVars_rfe.txt",sep = ""), sep="\t",row.names = TRUE,col.names=NA)

#### 7.10 Performance of optVars Classifier on  TestSet 019 #####################################################
#############################################################################################################

sink("TestSet019_ConfusionMatrix_rfe.txt", append = TRUE)
confusionMatrix(as.factor(Prediction_rfe.19$Prediction_rfe.19), as.factor(Prediction_rfe.19$TrueLabel))
sink()

sink("TestSet019_ConfusionMatrix_GA.txt", append = TRUE)
...
#### 7.11 ROC on probability "transforming" TestSet 019 ########################################################

Prediction_rfe.19$Class <- as.factor(ifelse(Prediction_rfe.19$TrueLabel == "transforming","transforming","nontransforming"))
roc.rfe.19 <- roc(Prediction_rfe.19$Class,                    
                  Prediction_rfe.19$transforming,             
                  percent=TRUE, levels=c("nontransforming","transforming"),
                  plot=T, auc.polygon=F, max.auc.polygon=F, col = "#000000", grid=F,
                  print.auc=T)
...
#### 7.12 Precision-Recall curve on probability "transforming" TestSet 019 ####################################
Prediction_rfe.19$Class_Code <- ifelse(Prediction_rfe.19$TrueLabel == "transforming",1,0)
pr.rfe.19 <- pr.curve(scores.class0 = Prediction_rfe.19$transforming , weights.class0 = Prediction_rfe.19$Class_Code, curve = TRUE,rand.compute = T)
plot(pr.rfe.19,rand.plot = TRUE, legend = F, color = 1, main = "")
...
#############################################################################################################################################
#############################################################################################################################################
#### Analyze all results from the different Test/Training iterations ########################################################################
#############################################################################################################################################
#############################################################################################################################################

#############################################################################################################################################
### 1. Summary of features, resampling- and test set accuracies for all 19 batchwise iterations: Supplementary Data 5 tab 1 #################
#############################################################################################################################################
library(gmodels)
...
df.mean   <- apply(Results,mean,MARGIN = 2,na.rm=TRUE)
df.median <- apply(Results,median,MARGIN = 2,na.rm=TRUE)

write.table(Results, file = "Results_19 TestSets_batchwise.txt", sep="\t",col.names=NA)

### 1.2 boxplots of results: Supplementary Figure 4 #######################################################
###########################################################################################################
library(reshape2)

## boxplot of feature numbers: Supplementary Figure 4a 
df.features <- melt(Results[,c(1,2,4,7)], id = "Testset", variable.name = "Source", value.name = "Number")
ggplot(df.features, aes(x=Source, y=Number)) +  
  geom_boxplot(fill="#E8E8E8") + 
...
## boxplot of test set accuracy: Supplementary Figure 4b 
df.acc      <- melt(Results[,c(1,9,12)], id = "Testset", variable.name = "Source", value.name = "Accuracy")
ggplot(df.acc, aes(x=Source, y=Accuracy)) +  
      geom_boxplot(fill="#E8E8E8") + 
      geom_dotplot(binaxis='y', stackdir='center', dotsize=0.7) +
      theme_classic() + 
      theme(axis.title.x = element_text(size=17),axis.title.y = element_text(size=17),
...
## boxplot of AUCROC on test sets Supplementary Figure 4c 
df.AUCROC <- melt(Results[,c(1,10,13)], id = "Testset", variable.name = "Source", value.name = "AUCROC")
ggplot(df.AUCROC, aes(x=Source, y=AUCROC)) +  
  geom_boxplot(fill="#E8E8E8") + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=0.7) +
  theme_classic() + 
  theme(axis.title.x = element_text(size=17),axis.title.y = element_text(size=17),
...
## boxplot of AUCPRC on test sets Supplementary Figure 4d
df.AUCPRC <- melt(Results[,c(1,11,14,15)], id = "Testset", variable.name = "Source", value.name = "AUCPRC")
ggplot(df.AUCPRC, aes(x=Source, y=AUCPRC)) +  
  geom_boxplot(fill="#E8E8E8") + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=0.7) +
  theme_classic() + 
  theme(axis.title.x = element_text(size=17),axis.title.y = element_text(size=17),
...
## boxplot of cross-validation accuracies (not shown): 
df.acc.cv <- melt(Results[,c(1,3,5,8)], id = "Testset", variable.name = "Source", value.name = "CVAccuracy")
ggplot(df.acc.cv, aes(x=Source, y=CVAccuracy)) +  
  geom_boxplot(fill="#E8E8E8") + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=0.7) +
  theme_classic() + 
  theme(axis.title.x = element_text(size=17),axis.title.y = element_text(size=17),
...
t.test(Results$Accuracy_TestSet_full,Results$Accuracy_TestSet_SAGA,paired = T)


#############################################################################################################################################
### 2. aggregate Test set performances over the 19 batchwise iterations #####################################################################
#############################################################################################################################################

### 2.1 TestSet performances for all 19 SVM_full models   ###################################################################################
...
### 2.2 TestSet performances for all 19 rfe models   #########################################################################################
...
colnames(Prediction_rfe.11)[7] <- "Prediction" 
colnames(Prediction_rfe.12)[7] <- "Prediction" 
colnames(Prediction_rfe.13)[7] <- "Prediction" 
colnames(Prediction_rfe.14)[7] <- "Prediction" 
colnames(Prediction_rfe.15)[7] <- "Prediction" 
colnames(Prediction_rfe.16)[7] <- "Prediction" 
colnames(Prediction_rfe.17)[7] <- "Prediction" 
colnames(Prediction_rfe.18)[7] <- "Prediction" 
colnames(Prediction_rfe.19)[7] <- "Prediction" 
...
Predictions_RFE   <- rbind(Prediction_rfe.1,Prediction_rfe.2, Prediction_rfe.3,Prediction_rfe.4,Prediction_rfe.5,
                           ...
                           Prediction_rfe.11,Prediction_rfe.12,Prediction_rfe.13,Prediction_rfe.14,Prediction_rfe.15,
                           Prediction_rfe.16,Prediction_rfe.17,Prediction_rfe.18,Prediction_rfe.19)

### 2.3 TestSet performances for all 18 SVM_GA models   #####################################################################################
...
Predictions_GA   <- rbind(Prediction_GA.1, Prediction_GA.3,Prediction_GA.4,Prediction_GA.5,
...
#############################################################################################################################################
### 2.5. Confusion matrices of aggregated TestSets: Supplementary Data 5 tab 2 ##############################################################
#############################################################################################################################################
...
### 2.6.2. IVIM vs SAGA AUROC (for all vectors) Figure 4d ###################################################################################
#############################################################################################################################################
...
roc.SAGA   <- roc( Predictions_SAGA$Class,                    
                   ...
                   percent=TRUE, levels=c("nontransforming","transforming"),
                   plot=T, auc.polygon=F, max.auc.polygon=F, col ="#ED7E23", grid=F,lwd = 4,
...
### 2.6.4. IVIM vs SAGA on LTR.RV.SF only: Figure 4e ###################################################################################################
#############################################################################################################################################
...
### 2.6.5. IVIM vs SAGA non-LTR vectors Figure 4f #############################################################################
#############################################################################################################################################
...
roc.test(ROC.SAGA.other, ROC.IVIM.other, alternative = "greater" )  # p = 0.01241


### 2.6.6. IVIM vs SAGA PRROC ###############################################################################################################
#############################################################################################################################################

### 2.6.6.1 for all vectors: Figure 4g  #####################################################################################################
...
### 2.6.2.3 for non-LTR.SF  Figure 4i #################################################################################################################
...
#############################################################################################################################################
### 2.7. Toplist of predictors chosen during the 19 iterations / Table 1 / Supplementary Data 5 tab 3 #######################################
#############################################################################################################################################
...
Results_complete <- data.frame(Testset = c(seq(1,19,by=1)), FeatureNumber_full = rep(NA,19), ResamplingAccuracy_full=rep(NA,19),CI_full_lower=rep(NA,19),CI_full_upper=rep(NA,19), 
                      FeatureNumber_rfe = rep(NA,19), ResamplingAccuracy_rfe = rep(NA,19), CI_rfe_lower=rep(NA,19),CI_rfe_upper=rep(NA,19), P_full_vs_rfe=rep(NA,19),
                      FeatureNumber_GA  = rep(NA,19), ResamplingAccuracy_GA = rep(NA,19),CI_GA_lower=rep(NA,19),CI_GA_upper=rep(NA,19),
                      Accuracy_TestSet_full = rep(NA,19), CI_Test_full_lower=rep(NA,19),CI_Test_full_upper=rep(NA,19),
                      Accuracy_TestSet_rfe = rep(NA,19),  CI_Test_rfe_lower=rep(NA,19),CI_Test_rfe_upper=rep(NA,19),
                      Accuracy_TestSet_GA = rep(NA,19),   CI_Test_GA_lower=rep(NA,19),CI_Test_GA_upper=rep(NA,19),
                      Accuracy_TestSet_SAGA = rep(NA,19),AUROC_SAGA = rep(NA,19), AUPRC_SAGA = rep(NA,19),AUPRC_random = rep(NA,19))

### 2.2. number of features selected by IQR filter for each training/test split
for (i in 1:19) {Results_complete[i,2] <- dim(eval(parse(text=paste("matrix.train.",i, sep=""))))[2] }

### 2.3. resampling accuracy for the full model / stored in svmFull.x$results dataframe
for (i in 1:19) { Results_complete[i,3] <- eval(parse(text=paste("svmFull.",i, sep="")))$results[eval(parse(text=paste("svmFull.",i, sep="")))$results$C == eval(parse(text=paste("svmFull.",i, sep="")))$bestTune$C,"Accuracy"] }
...
for (i in 1:19) {Results_complete[i,4] <- ci(eval(parse(text=paste("rfeResamples.",i, sep="")))$values[,2])[2]}
...
for (i in 1:19) {Results_complete[i,5] <- ci(eval(parse(text=paste("rfeResamples.",i, sep="")))$values[,2])[3]}

### 2.6. number of features found by SVM-rfe for each training/test split
for (i in 1:19) {Results_complete[i,6] <- eval(parse(text=paste("rfe.",i, sep="")))$optsize }

### 2.7. external resampling accuracy for the svm-rfe model
for (i in 1:19) { Results_complete[i,7] <- eval(parse(text=paste("rfe.",i, sep="")))$results[eval(parse(text=paste("rfe.",i, sep="")))$results$Variables == eval(parse(text=paste("rfe.",i, sep="")))$optsize,5] }
...
for (i in 1:19) {Results_complete[i,8] <- ci(eval(parse(text=paste("rfeResamples.",i, sep="")))$values[,7])[2]}

### 2.9. upper bound confidence intervall for the resampling accuracy of rfe model / stored in rfeResamples.x$values dataframe
for (i in 1:19) {Results_complete[i,9] <- ci(eval(parse(text=paste("rfeResamples.",i, sep="")))$values[,7])[3]}
...
for (i in 1:19) {Results_complete[i,10] <- eval(parse(text=paste("modelDifferences.",i, sep="")))$statistics$Accuracy[[1]][3]}

### 2.11. number of features found by SVM-GA 
for (i in c(1,3:19)) { Results_complete[i,11] <- length(eval(parse(text=paste("GA.",i, sep="")))$optVariables)  }

### 2.12. Resampling accuracy for the GA model
for (i in c(1,3:19)) { Results_complete[i,12] <- round(eval(parse(text=paste("accuracy.external.",i, sep=""))),3)}
...
for (i in c(1,3:19)) {Results_complete[i,13] <- ci(eval(parse(text=paste("accuracy.external.opt.",i, sep=""))))[2]}
...
for (i in c(1,3:19)) {Results_complete[i,14] <- ci(eval(parse(text=paste("accuracy.external.opt.",i, sep=""))))[3]}

### 2.15. TestSet accuracy for the full model
for (i in 1:19) {Results_complete[i,15] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_SVM_full.",i, sep="")))$Prediction_SVM_full), as.factor(eval(parse(text=paste("Prediction_SVM_full.",i, sep="")))$TrueLabel))$overall[1] }

### 2.16. lower bound of confidence intervall of the TestSet accuracy for the full model
for (i in 1:19) {Results_complete[i,16] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_SVM_full.",i, sep="")))$Prediction_SVM_full), as.factor(eval(parse(text=paste("Prediction_SVM_full.",i, sep="")))$TrueLabel))$overall[3] }

### 2.17. upper bound confidence intervall of the TestSet accuracy for the full model
for (i in 1:19) {Results_complete[i,17] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_SVM_full.",i, sep="")))$Prediction_SVM_full), as.factor(eval(parse(text=paste("Prediction_SVM_full.",i, sep="")))$TrueLabel))$overall[4] }

### 2.18. TestSet accuracy for the SVM-rfe model
for (i in 1:19) {Results_complete[i,18] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_rfe.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_rfe.",i, sep="")))$TrueLabel))$overall[1] }

### 2.19. lower bound of confidence intervall of the TestSet accuracy for the SVM-rfe model
for (i in 1:19) {Results_complete[i,19] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_rfe.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_rfe.",i, sep="")))$TrueLabel))$overall[3] }

### 2.20. upper bound confidence intervall of the TestSet accuracy for the SVM-rfe model
for (i in 1:19) {Results_complete[i,20] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_rfe.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_rfe.",i, sep="")))$TrueLabel))$overall[4] }

### 2.21. TestSet accuracy for the SVM-GA model
for (i in c(1,3:19)) {Results_complete[i,21] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[1] }

### 2.22. lower bound of confidence intervall TestSet accuracy for the SVM-GA model
for (i in c(1,3:19)) {Results_complete[i,22] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[3] }

### 2.23. upper bound of confidence intervall TestSet accuracy for the SVM-GA model
for (i in c(1,3:19)) {Results_complete[i,23] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[4] }

### 2.24.TestSet accuracy for SAGA = TestSet accuracy of rfe (TestSet 02; <10 features found by rfe) and GA otherwise (all other TestSets) 
for (i in c(1,3:19)) {Results_complete[i,24] <- confusionMatrix(as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_GA.",i, sep="")))$TrueLabel))$overall[1] }
Results_complete[2,24] <- round(confusionMatrix(as.factor(eval(parse(text=paste("Prediction_rfe.",2, sep="")))[,7]), as.factor(eval(parse(text=paste("Prediction_rfe.",2, sep="")))$TrueLabel))$overall[1],3)

### 2.25. TestSet AUROC for SAGA
for (i in c(1,3:19)) { Results_complete[i,25] <- round(eval(parse(text=paste("roc.GA.",i, sep="")))$auc[1]/100,3) }
Results_complete[2,25] <- round(roc.rfe.2$auc[1]/100,3)

### 2.26. TestSet AUPRC for SAGA
for (i in c(1,3:19)) { Results_complete[i,26] <- round(eval(parse(text=paste("pr.GA.",i, sep="")))$auc.integral,3)}
Results_complete[2,26] <- round(pr.rfe.2$auc.integral,3)

### 2.27. TestSet AUPRC for a random classifier
for (i in c(1,3:19)) { Results_complete[i,27] <- round(eval(parse(text=paste("pr.GA.",i, sep="")))$rand$auc.integral,3)}
Results_complete[2,27] <- round(eval(parse(text=paste("pr.rfe.",2, sep="")))$rand$auc.integral,3)
...
Accuracy_TestSet_full.median   <- median(Results_complete$Accuracy_TestSet_full)
ResamplingAccuracy_full.median <- median(Results_complete$ResamplingAccuracy_full)

ggplot(Results_complete, aes(Accuracy_TestSet_full,ResamplingAccuracy_full, label = Testset)) +
  geom_point(size=7) +
...
Accuracy_TestSet_rfe.median   <- median(Results_complete$Accuracy_TestSet_rfe)
ResamplingAccuracy_rfe.median <- median(Results_complete$ResamplingAccuracy_rfe)

ggplot(Results_complete, aes(Accuracy_TestSet_rfe,ResamplingAccuracy_rfe, label = Testset)) +
  geom_point(size=7) +
...
df.results.GA              <- Results_complete[c(1,3:19),]    # subset for the Training / TestSet splits in which GA was performed 
